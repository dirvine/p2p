...
use saorsa_core::{
    ...
    error::{P2PError as P2PError, SecurityError},
};
...
async fn test_identity_encryption_at_rest() -> Result<(), P2PError> {
    ...
    assert!(!raw_str.contains("Test User"), "Display name should be encrypted");
    assert!(!raw_str.contains("Test bio with sensitive info"), "Bio should be encrypted");
    assert!(!raw_str.contains(&identity_id.to_string()), "Identity ID should be encrypted");
...
async fn test_key_rotation_updates_encryption() -> Result<(), P2PError> {
    ...
    assert_ne!(original_data, rotated_data, "Encrypted data should change after rotation");
...
async fn test_migrate_plaintext_identity() -> Result<(), P2PError> {
    ...
    assert!(!identity_path.exists(), "Old plaintext file should be removed");
    ...
    assert!(!migrated_str.contains("Plaintext User"), "Should be encrypted after migration");
    assert!(!migrated_str.contains("This is currently in plaintext"), "Bio should be encrypted");
    
    // Verify we can load and decrypt
    let loaded = manager.load_identity(&identity_id, &password).await?;
    assert_eq!(loaded.display_name, Some("Plaintext User".to_string()));
    assert_eq!(loaded.bio, Some("This is currently in plaintext".to_string()));
...
async fn test_encryption_performance() -> Result<(), P2PError> {
    ...
    assert!(create_duration.as_millis() < 50, "Create operation took {}ms, should be < 50ms", create_duration.as_millis());
    assert!(load_duration.as_millis() < 50, "Load operation took {}ms, should be < 50ms", load_duration.as_millis());
...
async fn test_concurrent_encrypted_access() -> Result<(), P2PError> {
    let temp_dir = TempDir::new().unwrap();
    let manager = std::sync::Arc::new(IdentityManager::new(
        temp_dir.path(),
        SecurityLevel::Fast,
    ).await?);
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            for _ in 0..5 {
                let loaded = manager_clone.load_identity(&id_clone, &password_clone).await?;
...
async fn test_encryption_format_versioning() -> Result<(), P2PError> {
    ...
    assert!(encrypted_data.len() > 4 + 32 + 12, "Encrypted data should have version, salt, nonce, and ciphertext");
