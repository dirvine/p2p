...
impl NetworkSimulator {
    fn new() -> Self {
        ...
        success_rates.insert((StrategyChoice::Kademlia, ContentType::RealtimeMessage), 0.40);
        success_rates.insert((StrategyChoice::Kademlia, ContentType::ComputeRequest), 0.70);
        ...
        success_rates.insert((StrategyChoice::Hyperbolic, ContentType::DataRetrieval), 0.90);
        success_rates.insert((StrategyChoice::Hyperbolic, ContentType::RealtimeMessage), 0.95);
        success_rates.insert((StrategyChoice::Hyperbolic, ContentType::ComputeRequest), 0.75);

        // TrustPath: Good for compute requests, poor for DHT
        success_rates.insert((StrategyChoice::TrustPath, ContentType::DHTLookup), 0.50);
        success_rates.insert((StrategyChoice::TrustPath, ContentType::DataRetrieval), 0.75);
        success_rates.insert((StrategyChoice::TrustPath, ContentType::RealtimeMessage), 0.80);
        success_rates.insert((StrategyChoice::TrustPath, ContentType::ComputeRequest), 0.95);

        // SOMRegion: Specialized for compute requests
        success_rates.insert((StrategyChoice::SOMRegion, ContentType::DHTLookup), 0.60);
        success_rates.insert((StrategyChoice::SOMRegion, ContentType::DataRetrieval), 0.70);
        success_rates.insert((StrategyChoice::SOMRegion, ContentType::RealtimeMessage), 0.65);
        success_rates.insert((StrategyChoice::SOMRegion, ContentType::ComputeRequest), 0.98);
    ...
    fn simulate_request(
        ...
        let (min_latency, max_latency) = self.latencies
            .get(&strategy)
            .unwrap_or(&(50, 200));
        
        let latency_ms = min_latency + 
            (rand::random::<u64>() % (max_latency - min_latency));
...
async fn test_mab_learns_optimal_strategies() {
    ...
    for _ in 0..1000 {
        for content_type in &[
            ...
            let decision = mab.select_route(
                &destination,
                *content_type,
                &all_strategies,
            ).await.unwrap();

            let outcome = simulator.simulate_request(
                decision.route_id.strategy,
                *content_type,
            );
    ...
    let kademlia_dht = strategy_selections
        ...
        .unwrap_or(&0);
    assert!(*kademlia_dht > 600, "Kademlia should be preferred for DHT lookups");
    ...
    let hyperbolic_realtime = strategy_selections
        ...
        .unwrap_or(&0);
    assert!(*hyperbolic_realtime > 600, "Hyperbolic should be preferred for real-time messages");
    ...
    let som_compute = strategy_selections
        ...
        .unwrap_or(&0);
    assert!(*som_compute > 600, "SOMRegion should be preferred for compute requests");
...
async fn test_mab_exploration_vs_exploitation() {
    ...
    assert!((exploration_ratio - 0.2).abs() < 0.05, 
        "Exploration ratio {} should be close to 0.2", exploration_ratio);
}
...
async fn test_mab_confidence_intervals() {
    ...
    let (lower, upper) = mab.get_route_confidence(&route_id, ContentType::DHTLookup)
        .await
        .unwrap();
    ...
    assert!(lower > 0.6 && lower < 0.8, "Lower bound {} should be around 0.7", lower);
    assert!(upper > 0.8 && upper < 0.95, "Upper bound {} should be around 0.9", upper);
...
async fn test_mab_adaptive_to_network_changes() {
    ...
    assert!(kademlia_count > 70, "Kademlia should be preferred initially");
    ...
    assert!(hyperbolic_count > 70, "Should adapt to prefer Hyperbolic after network change");
}