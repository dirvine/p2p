...
use saorsa_core::adaptive::{
    EvictionStrategy, EvictionStrategyType, LRUStrategy, LFUStrategy,
    AdaptiveStrategy, CacheState, AccessInfo, ContentHash, StateVector, CacheAction,
};
...
async fn test_strategy_switching() {
    ...
    test_data.insert(
        ...
        AccessInfo {
            count: 10,
            last_access_secs: SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs() - 10,
            size: 1024,
        },
    );
    test_data.insert(
        ...
        AccessInfo {
            count: 1,
            last_access_secs: SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs() - 5,
            size: 1024,
        },
    );
...
async fn test_cache_state_calculations() {
    ...
    let avg_frequency = access_data.values().map(|info| info.count).sum::<u64>() as f64 
        / access_data.len() as f64;
