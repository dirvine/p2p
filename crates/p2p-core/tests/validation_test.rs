...
fn test_rate_limiter() {
    ...
    // Should allow burst
    for i in 0..5 {
        assert!(limiter.check_ip(&ip).is_ok(), "Failed at burst request {}", i);
    }
    ...
    assert!(succeeded >= 1, "Should allow at least 1 more request after refill, but allowed {}", succeeded);
...
fn test_network_message_validation() {
    ...
    let future_msg = NetworkMessage {
        ...
        timestamp: std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .unwrap()
            .as_secs() + 600, // 10 minutes in future
    };
...
fn test_config_value_validation() {
    // Valid values
    assert_eq!(validate_config_value::<u32>("100", Some(1), Some(1000)).unwrap(), 100);
    assert_eq!(validate_config_value::<f64>("3.14", Some(0.0), Some(10.0)).unwrap(), 3.14);
...
fn test_sanitization() {
    ...
    assert_eq!(sanitize_string("user_name-123.test", 20), "user_name-123.test");
    assert_eq!(sanitize_string("very_long_string_that_exceeds_limit", 10), "very_long_");
}
