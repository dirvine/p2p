...
mod gossipsub_tests {
    ...
    impl TrustProvider for MockTrustProvider {
        ...
        fn get_global_trust(&self) -> HashMap<NodeId, f64> {
            futures::executor::block_on(async {
                self.trust_scores.read().await.clone()
            })
        }
    ...
    async fn test_topic_prioritization() {
        ...
        gossipsub.set_topic_priority("high_priority", TopicPriority::Critical).await;
        gossipsub.set_topic_priority("medium_priority", TopicPriority::Normal).await;
        gossipsub.set_topic_priority("low_priority", TopicPriority::Low).await;
        ...
        let critical_size = gossipsub.calculate_adaptive_mesh_size("high_priority").await;
        let normal_size = gossipsub.calculate_adaptive_mesh_size("medium_priority").await;
