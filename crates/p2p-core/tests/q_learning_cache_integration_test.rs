        ...
        let idx = match &self.pattern {
            ...
            WorkloadPattern::Temporal { window } => {
                if self.access_history.len() > *window && rand::random::<f64>() < 0.8 {
                    ...
                    let recent_idx = self.access_history.len() - 1 - (rand::random::<usize>() % window);
                    let recent_hash = &self.access_history[recent_idx];
                    self.content_pool.iter().position(|(h, _)| h == recent_hash).unwrap()
                } else {
                    rand::random::<usize>() % self.content_pool.len()
                }
            }
...
async fn test_q_learning_improves_hit_rate() {
    ...
    for _ in 0..5000 {
        ...
        let actions = q_manager.get_available_actions(&content_hash, content_size)
            .await
            .unwrap();
        ...
        q_manager.update_statistics(&action, &content_hash, content_size, hit)
            .await
            .unwrap();
        ...
        // Calculate reward
        let reward = q_manager.calculate_reward(
            &action,
            hit,
            old_utilization,
            new_utilization,
    ...
    for _ in 0..1000 {
        ...
        let hit = q_manager.cache_stats.read().await
            .access_frequency.contains_key(&content_hash);
...
async fn test_q_learning_adapts_to_workload_changes() {
    ...
    for _ in 0..1000 {
        ...
        let reward = q_manager.calculate_reward(&action, hit, old_utilization, old_utilization).await;
        let next_state = q_manager.get_current_state(&content_hash).await.unwrap();
        
        q_manager.add_experience(Experience {
            state,
            action,
            reward,
            next_state,
    ...
    let mut workload = WorkloadGenerator::new(
        WorkloadPattern::Temporal { window: 10 },
        50,
    );
    ...
    println!("Sequential workload hit rate: {:.2}%", phase1_hit_rate * 100.0);
    println!("Temporal locality hit rate: {:.2}%", phase2_hit_rate * 100.0);
...
async fn test_q_learning_handles_mixed_content_sizes() {
    ...
    for i in 0..1000 {
        ...
        let reward = q_manager.calculate_reward(&action, hit, old_utilization, new_utilization).await;
        let next_state = q_manager.get_current_state(&content_hash).await.unwrap();
    ...
    println!("Final cache utilization: {:.2}%", final_stats.utilization() * 100.0);
...
async fn test_q_learning_convergence() {
    ...
    println!("Final epsilon: {:.4}", *q_manager.current_epsilon.read().await);
