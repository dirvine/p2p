...
use saorsa_core::identity::cli::{IdentityCommand, IdentityCliHandler};
...
mod cli_command_tests {
    ...
    async fn test_cli_sign_command() {
        ...
        tokio::fs::write(&message_file, b"Test message to sign").await.unwrap();
    ...
    async fn test_cli_error_handling() {
        ...
        assert!(result.unwrap_err().to_string().contains("No identity found"));
    }
    ...
    async fn test_cli_generate_with_seed() {
        ...
        let id1 = saorsa_core::identity::NodeIdentity::load_from_file(&identity_path).await.unwrap();
        let id2 = saorsa_core::identity::NodeIdentity::load_from_file(&identity_path2).await.unwrap();
        
        // Should be identical
        assert_eq!(id1.node_id(), id2.node_id());
        assert_eq!(id1.word_address().to_string(), id2.word_address().to_string());
    }
}
