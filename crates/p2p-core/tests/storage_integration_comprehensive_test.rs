...
use saorsa_core::{
    Config, Node, StorageKey, StorageValue,
...
impl StorageTestFramework {
    ...
    async fn store_test_data(&self, key: StorageKey, value: StorageValue) -> Result<()> {
        ...
        self.nodes[0].store(key.clone(), value.clone()).await
...
async fn test_basic_store_and_retrieve() -> Result<()> {
    ...
    framework.store_test_data(key.clone(), value.clone()).await?;
...
async fn test_replication_factor_compliance() -> Result<()> {
    ...
    for i in 0..10 {
        ...
        assert\!(replica_count >= replication_factor, 
                "Key {} has {} replicas, expected >= {}", 
                i, replica_count, replication_factor);
    }
...
async fn test_storage_consistency_under_load() -> Result<()> {
    ...
    let store_tasks = (0..20).map(|i| {
        ...
        async move {
            ...
            framework.nodes[node_index].store(key.clone(), value.clone()).await?;
    ...
    assert\!(is_consistent, "Storage inconsistency detected under concurrent load");
...
async fn test_storage_recovery_after_node_failure() -> Result<()> {
    ...
    for (key, expected_value) in test_records.iter() {
        ...
        for (i, node) in framework.nodes.iter().enumerate() {
            if i == 1 { continue; } // Skip failed node
...
async fn test_large_data_storage_and_retrieval() -> Result<()> {
    ...
    for size in test_sizes {
        ...
        let retrieved_value = framework.nodes[0].retrieve(&key).await?
        ...
        println\!("Size: {}B, Store: {:?}, Retrieve: {:?}", 
                size, store_duration, retrieve_duration);
        
        // Performance assertions (adjust based on requirements)
        assert\!(store_duration.as_millis() < 5000, "Store operation too slow");
        assert\!(retrieve_duration.as_millis() < 1000, "Retrieve operation too slow");
    }
...
async fn test_storage_space_management() -> Result<()> {
    ...
    for i in 0..90 { // Try to store 90MB (near the 100MB limit)
    ...
    assert\!(stored_keys.len() > 50, "Should be able to store at least 50MB");
    assert\!(stored_keys.len() < 100, "Should hit storage limits before 100MB");
    
    // Verify storage stats
    let stats = framework.get_storage_stats().await?;
    for (node_id, record_count, total_size) in stats {
        println\!("Node {}: {} records, {} bytes", node_id, record_count, total_size);
        assert\!(total_size > 50 * 1024 * 1024, "Node should have significant storage");
        assert\!(total_size <= 100 * 1024 * 1024, "Node should respect storage limits");
    }
...
async fn test_concurrent_read_write_performance() -> Result<()> {
    ...
    assert\!(ops_per_second > 10.0, "Performance should be > 10 ops/second");
