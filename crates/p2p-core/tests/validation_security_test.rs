...
fn test_dos_protection_rate_limiting() {
    ...
    assert!(blocked > 80, "Rate limiting not effective: {} blocked out of 100", blocked);
...
fn test_timing_attack_resistance() -> Result<()> {
    ...
    let valid_avg: u128 = valid_times.iter().map(|d| d.as_nanos()).sum::<u128>() / valid_times.len() as u128;
    let invalid_avg: u128 = invalid_times.iter().map(|d| d.as_nanos()).sum::<u128>() / invalid_times.len() as u128;
