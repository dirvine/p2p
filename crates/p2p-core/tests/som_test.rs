...
mod feature_tests {
    ...
    fn test_feature_normalization() {
        ...
        let magnitude: f64 = normalized.content_vector.iter()
            .map(|x| x * x)
            .sum::<f64>()
            .sqrt();
        assert!((magnitude - 1.0).abs() < 1e-6, "Content vector should be unit length");
    ...
    fn test_feature_distance() {
        ...
        assert!(distance > 0.0, "Different features should have positive distance");
...
mod bmu_tests {
    ...
    fn test_bmu_performance() {
        ...
        assert!(avg_time < 1000.0, "BMU search should complete in < 1ms for 1000 neurons");
    }
}
...
mod learning_tests {
    ...
    fn test_weight_update() {
        ...
        assert!(updated_distance < initial_distance, "Weights should move towards input");
    }
    ...
    fn test_batch_training() {
        ...
        assert!(distance <= 2, "Similar features should map to nearby neurons");
    }
...
mod node_assignment_tests {
    ...
    fn test_node_assignment() {
        ...
        assert!(assigned_nodes.contains(&node_id), "Node should be assigned to its BMU");
    }
    ...
    fn test_node_reassignment() {
        ...
        assert!(!old_nodes.contains(&node_id), "Node should be removed from old neuron");
        
        // Check new neuron has the node
        let new_nodes = som.get_assigned_nodes(new_x, new_y);
        assert!(new_nodes.contains(&node_id), "Node should be assigned to new neuron");
    }
    ...
    fn test_find_similar_nodes() {
        ...
        assert!(similar_nodes.len() <= 10, "Should not return more nodes than assigned");
    }
}
...
mod grid_sizing_tests {
    ...
    fn test_dynamic_grid_sizing() {
        ...
        assert!(width <= 20 && height <= 20, "Grid should respect maximum size");
    }
    ...
    fn test_grid_resize_preserves_assignments() {
        ...
        for node_id in &node_ids {
            assert!(all_assigned.contains(node_id), "Original nodes should remain assigned after resize");
        }
    }
}
...
mod visualization_tests {
    ...
    fn test_grid_visualization() {
        ...
        let assigned_count = viz_data.neurons.iter()
            .filter(|n| !n.assigned_nodes.is_empty())
            .count();
        assert!(assigned_count > 0, "Some neurons should have assigned nodes");
    }
...
mod integration_tests {
    ...
    async fn test_full_som_workflow() {
        ...
        assert!(compute_cluster_spread < 5.0, "Compute nodes should cluster together");
        assert!(storage_cluster_spread < 5.0, "Storage nodes should cluster together");
        ...
        assert!(inter_cluster_distance > 2.0, "Different clusters should be separated");
    }
    ...
    fn calculate_cluster_spread(positions: &[(usize, usize)]) -> f64 {
        ...
        let center_x = positions.iter().map(|(x, _)| *x as f64).sum::<f64>() / positions.len() as f64;
        let center_y = positions.iter().map(|(_, y)| *y as f64).sum::<f64>() / positions.len() as f64;
...
mod benchmark_tests {
    ...
    fn benchmark_training_performance() {
        ...
        assert!(elapsed.as_secs() < 10, "Training should complete within 10 seconds");
    }
