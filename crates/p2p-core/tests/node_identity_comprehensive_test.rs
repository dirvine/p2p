...
mod four_word_address_tests {
    ...
    fn test_four_word_address_deterministic() {
        ...
        let addr1 = FourWordAddress::from_node_id(node_id).expect("Should create address from node ID");
        let addr2 = FourWordAddress::from_node_id(node_id).expect("Should create address from node ID");
...
mod proof_of_work_tests {
    ...
    fn test_proof_of_work_difficulty() {
        ...
        assert!(leading_zeros >= difficulty, 
            "Expected {} leading zeros, got {}", difficulty, leading_zeros);
    }
...
mod persistence_tests {
    ...
    async fn test_identity_save_and_load() {
        ...
        assert_eq!(identity.word_address().to_string(), loaded.word_address().to_string());
        assert_eq!(identity.public_key().as_bytes(), loaded.public_key().as_bytes());
