...
use saorsa_core::health::{
    HealthManager, HealthStatus, ComponentChecker, HealthServer,
    NetworkHealthChecker, DhtHealthChecker, StorageHealthChecker,
    ResourceHealthChecker, TransportHealthChecker, PeerHealthChecker,
...
async fn test_health_manager_caching() {
    ...
    manager.register_checker(
        "counting",
        Box::new(CountingChecker { counter: counter_clone }),
    ).await;
...
async fn test_network_health_checker() {
    ...
    let checker = NetworkHealthChecker::new(|| async { Ok(10) })
        .with_min_peers(5);
    ...
    let checker = NetworkHealthChecker::new(|| async { Ok(2) })
        .with_min_peers(5);
    ...
    let checker = NetworkHealthChecker::new(|| async { Ok(0) })
        .with_min_peers(1);
...
async fn test_dht_health_checker() {
    ...
    let checker = DhtHealthChecker::new(|| async { Ok(20) })
        .with_min_nodes(10);
    ...
    let checker = DhtHealthChecker::new(|| async { Ok(5) })
        .with_min_nodes(10);
...
async fn test_storage_health_checker() {
    ...
    let checker = StorageHealthChecker::new(temp_dir.path().to_path_buf())
        .with_min_free_space(1024); // 1KB minimum
...
async fn test_peer_health_checker() {
    ...
    let checker = PeerHealthChecker::new(|| async { Ok(50) })
        .with_peer_limits(10, 100);
    
    let status = checker.check().await.unwrap();
    assert_eq!(status, HealthStatus::Healthy);
    
    // Test with too few peers
    let checker = PeerHealthChecker::new(|| async { Ok(5) })
        .with_peer_limits(10, 100);
    ...
    let checker = PeerHealthChecker::new(|| async { Ok(150) })
        .with_peer_limits(10, 100);
...
async fn test_composite_health_checker() {
    let checker = CompositeHealthChecker::new()
        .add_checker("healthy", Box::new(TestChecker {
            status: HealthStatus::Healthy,
            latency_ms: 5,
        }))
        .add_checker("degraded", Box::new(TestChecker {
            status: HealthStatus::Degraded,
            latency_ms: 5,
        }))
        .add_checker("healthy2", Box::new(TestChecker {
            status: HealthStatus::Healthy,
            latency_ms: 5,
        }));
    ...
    let checker = CompositeHealthChecker::new()
        .add_checker("unhealthy", Box::new(TestChecker {
            status: HealthStatus::Unhealthy,
            latency_ms: 5,
        }));
...
async fn test_health_check_timeout() {
    ...
    let manager = HealthManager::new("1.0.0-test".to_string());
    manager.register_checker("timeout", Box::new(TimeoutChecker)).await;
