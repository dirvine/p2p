...
use saorsa_core::{
    Config, Node, NetworkEvent, PeerRecord, Identity,
    ...
    async fn simulate_network_partition(&self, partition_point: usize) -> Result<()> {
        ...
        for i in 0..partition_point {
            for j in partition_point..self.nodes.len() {
                self.nodes[i].disconnect_peer(&format\!("node_{}", j)).await?;
                self.nodes[j].disconnect_peer(&format\!("node_{}", i)).await?;
            }
        }
...
async fn test_multi_node_startup_and_discovery() -> Result<()> {
    ...
    for (node_id, peer_count) in &stats {
        if *node_id == 0 || *node_id == stats.len() - 1 {
            assert\!(peer_count >= &1, "Endpoint node {} should have at least 1 peer", node_id);
        } else {
            assert\!(peer_count >= &2, "Middle node {} should have at least 2 peers", node_id);
        }
    }
...
async fn test_network_partition_and_healing() -> Result<()> {
    ...
    for (node_id, peer_count) in &initial_stats {
        assert\!(peer_count >= &4, "Node {} should have at least 4 peers initially", node_id);
    }
    ...
    for (node_id, peer_count) in &partition_stats {
        if *node_id < 3 {
            assert\!(peer_count <= &2, "Node {} in partition A should have <= 2 peers", node_id);
        } else {
            assert\!(peer_count <= &2, "Node {} in partition B should have <= 2 peers", node_id);
        }
    }
    ...
    for (node_id, peer_count) in &healed_stats {
        assert\!(peer_count >= &3, "Node {} should have at least 3 peers after healing", node_id);
    }
...
async fn test_peer_discovery_under_load() -> Result<()> {
    ...
    assert\!(total_connections >= 18, "Should maintain at least star topology connections");
...
async fn test_connection_failure_recovery() -> Result<()> {
    ...
    assert\!(failure_total < initial_total, "Total connections should decrease after node failure");
    ...
    for (node_id, peer_count) in &failure_stats {
        if *node_id == 0 || *node_id == 2 {
            ...
            assert\!(peer_count < &2, "Node {} should have detected failure", node_id);
        }
    }
...
async fn test_high_throughput_messaging() -> Result<()> {
    ...
    for i in 1..framework.nodes.len() {
        for msg_id in 0..message_count {
            ...
            framework.nodes[0].send_message(&peer_id, message).await
    ...
    assert\!(throughput > 10.0, "Throughput should be > 10 messages/second");
