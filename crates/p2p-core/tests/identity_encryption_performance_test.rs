...
async fn test_encryption_performance_overhead() {
    ...
    for _ in 0..100 {
        ...
        let start = Instant::now();
        let _ = manager2.import_sync_package(
            &sync_package,
            &device_password,
            &storage_password
        ).await.unwrap();
...
async fn test_encryption_with_different_data_sizes() {
    ...
    for (name, size) in sizes {
        ...
        println!("  Encrypted size: {} bytes", sync_package.encrypted_identity.len());
        println!("  Overhead: {} bytes", sync_package.encrypted_identity.len() - size);
