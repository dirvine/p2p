...
use saorsa_core::{
    Config, Node, Identity, SecureMessage,
    security::{SecurityManager, EncryptionKey, SigningKey},
...
impl SecurityTestFramework {
    async fn new(node_count: usize, malicious_count: usize) -> Result<Self> {
        ...
        for i in 0..malicious_count {
            ...
            let identity = Identity::generate()?;
            let node = Node::new_with_identity(config, identity).await
    ...
    async fn start_legitimate_nodes(&self) -> Result<()> {
        ...
        for i in 0..self.nodes.len() {
            for j in (i+1)..self.nodes.len() {
                ...
                self.nodes[i].connect_to_peer_secure(&peer_addr, &self.identities[j]).await?;
    ...
    async fn simulate_authentication_attack(&self) -> Result<Vec<bool>> {
        ...
        for (i, malicious_node) in self.malicious_nodes.iter().enumerate() {
            ...
            for j in 0..self.nodes.len() {
                ...
                if attack_succeeded {
                    println\!("⚠️ Authentication bypass detected from malicious node {}", i);
                }
            }
        }
    ...
    async fn test_message_encryption(&self) -> Result<bool> {
        ...
        let encrypted_msg = sender.encrypt_message(&message, &self.identities[1]).await?;
        
        // Verify message is actually encrypted (not plaintext)
        let is_encrypted = encrypted_msg.ciphertext \!= message;
        
        // Send the encrypted message
        sender.send_encrypted_message(&self.identities[1].peer_id(), encrypted_msg.clone()).await?;
        
        // Receiver should be able to decrypt
        let decrypted = receiver.decrypt_message(encrypted_msg, &self.identities[0]).await?;
    ...
    async fn simulate_man_in_the_middle_attack(&self) -> Result<bool> {
        ...
        match receiver.decrypt_message(modified_msg, &self.identities[0]).await {
            Ok(decrypted) => {
    ...
    async fn test_replay_attack_protection(&self) -> Result<bool> {
        ...
        let first_result = receiver.verify_and_process_message(signed_msg.clone()).await;
    ...
    async fn test_access_control(&self) -> Result<Vec<bool>> {
        ...
        admin_node.set_access_policy("admin_resource", vec\![self.identities[0].peer_id()]).await?;
        admin_node.set_access_policy("public_resource", 
                                   self.identities.iter().map(|id| id.peer_id()).collect()).await?;
        ...
        for malicious_node in &self.malicious_nodes {
            let admin_access = malicious_node.access_resource("admin_resource").await.is_ok();
            let public_access = malicious_node.access_resource("public_resource").await.is_ok();
    ...
    async fn performance_benchmark_encryption(&self) -> Result<(f64, f64)> {
        ...
        for size in message_sizes {
            ...
            for _ in 0..iterations {
                ...
                let encrypted = sender.encrypt_message(&message, &self.identities[1]).await?;
                let encrypt_duration = encrypt_start.elapsed();
                
                // Measure decryption time
                let decrypt_start = std::time::Instant::now();
                let _ = receiver.decrypt_message(encrypted, &self.identities[0]).await?;
...
async fn test_message_encryption_integrity() -> Result<()> {
    ...
    for i in 0..5 {
        ...
        let encrypted = framework.nodes[sender_idx]
            .encrypt_message(&message, &framework.identities[receiver_idx]).await?;
        
        let decrypted = framework.nodes[receiver_idx]
            .decrypt_message(encrypted, &framework.identities[sender_idx]).await?;
        
        assert_eq\!(decrypted, message, "Message {} encryption/decryption failed", i);
    }
...
async fn test_crypto_performance() -> Result<()> {
    ...
    assert\!(avg_encrypt_ms < 50.0, "Encryption should be < 50ms on average");
    assert\!(avg_decrypt_ms < 50.0, "Decryption should be < 50ms on average");
    ...
    for i in 0..framework.nodes.len() {
        for j in 0..framework.nodes.len() {
            if i \!= j {
                ...
                let verification_result = verifier.verify_signature(
                    &message, 
                    &signature, 
                    &identity_to_verify.public_key()
                ).await?;
                
                assert\!(verification_result, 
                       "Node {} should verify identity of node {}", i, j);
            }
        }
    }
...
async fn test_secure_group_communication() -> Result<()> {
    ...
    for i in 1..3 {
        framework.nodes[0].share_group_key(&framework.identities[i], &group_key).await?;
    }
    
    // Test group message encryption/decryption
    let group_message = b"secret_group_message".to_vec();
    let encrypted_group_msg = framework.nodes[0].encrypt_group_message(&group_message, &group_key).await?;
    
    // All group members should be able to decrypt
    for i in 0..3 {
        let decrypted = framework.nodes[i].decrypt_group_message(&encrypted_group_msg, &group_key).await?;
        assert_eq\!(decrypted, group_message, "Group member {} should decrypt message", i);
    }
    
    // Non-group members should NOT be able to decrypt
    for i in 3..framework.nodes.len() {
        let decrypt_result = framework.nodes[i].decrypt_group_message(&encrypted_group_msg, &group_key).await;
        assert\!(decrypt_result.is_err(), "Non-group member {} should not decrypt message", i);
    }
