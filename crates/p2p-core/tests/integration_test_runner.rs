...
impl IntegrationTestRunner {
    ...
    pub async fn run_all_tests(&mut self) -> Result<bool> {
        ...
        for (category, test_future) in test_categories {
            ...
            if category_passed {
                println\!("✅ {} tests PASSED ({:.2}s)", category, category_duration.as_secs_f64());
            } else {
                println\!("❌ {} tests FAILED ({:.2}s)", category, category_duration.as_secs_f64());
                all_passed = false;
            }
        }
    ...
    async fn run_network_tests(&mut self) -> Result<bool> {
        ...
        self.run_test_category("network_integration_comprehensive_test", &network_tests).await
    }
    ...
    async fn run_storage_tests(&mut self) -> Result<bool> {
        ...
        self.run_test_category("storage_integration_comprehensive_test", &storage_tests).await
    }
    ...
    async fn run_security_tests(&mut self) -> Result<bool> {
        ...
        self.run_test_category("security_integration_comprehensive_test", &security_tests).await
    }
    ...
    async fn run_e2e_tests(&mut self) -> Result<bool> {
        ...
        self.run_test_category("end_to_end_scenarios_test", &e2e_tests).await
    }
    ...
    fn format_test_report(&self) -> String {
        ...
        report.push_str(&format\!("- **Average per Test**: {:.2}s\n\n", total_duration.as_secs_f64() / total_tests as f64));
        
        report.push_str("## Test Results\n\n");
        
        for result in &self.results {
            let status = if result.success { "✅ PASS" } else { "❌ FAIL" };
            report.push_str(&format\!("### {} - {} ({:.2}s)\n\n", result.name, status, result.duration.as_secs_f64()));
        ...
        if failed_tests > 0 {
            ...
            report.push_str("Please review and fix the failed tests before proceeding to production.\n\n");
        }
        
        if total_duration.as_secs() > 600 { // 10 minutes
            report.push_str("### Performance\n");
            report.push_str("Test suite execution time is quite long. Consider optimizing slower tests.\n\n");
        }
