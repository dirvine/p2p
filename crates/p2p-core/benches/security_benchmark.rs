...
fn ipv6_identity_benchmarks(c: &mut Criterion) {
    ...
    for (i, addr) in ipv6_addrs.iter().enumerate() {
        group.bench_with_input(
            ...
            |b, addr| {
                b.iter(|| {
                    let node_id = IPv6NodeID::generate(black_box(*addr), black_box(&keypair)).unwrap();
                    black_box(node_id)
                });
            },
        );
...
fn ip_diversity_benchmarks(c: &mut Criterion) {
    ...
    let test_addresses = vec![
        "2001:db8:85a3:1234:5678:8a2e:370:7334".parse::<Ipv6Addr>().unwrap(),
...
fn concurrent_security_benchmarks(c: &mut Criterion) {
    ...
    let enforcer = Arc::new(Mutex::new(IPDiversityEnforcer::new(IPDiversityConfig::default())));
                    ...
                    let handles: Vec<_> = (0..thread_count)
                        .map(|i| {
                            ...
                            thread::spawn(move || {
                                ...
                                manager.update_reputation(&peer_id, i % 2 == 0, Duration::from_millis(100));
                            })
                        })
                        .collect();
...
fn security_crypto_benchmarks(c: &mut Criterion) {
    ...
    group.bench_function("node_id_verification", |b| {
        b.iter(|| {
            let result = keypair.public.verify(black_box(&data_to_verify), black_box(&signature));
            black_box(result)
        });
    });
