...
fn benchmark_lookup(c: &mut Criterion) {
    ...
    group.bench_function("parallel_lookups_10", |b| {
        b.to_async(&rt).iter(|| async {
            let futures: Vec<_> = test_data.iter()
                .take(10)
...
fn benchmark_storage(c: &mut Criterion) {
    ...
    let client = rt.block_on(async {
        Client::connect(ClientConfig::default()).await.unwrap()
    });
    
    // Benchmark different data sizes
    for size in [1_024, 10_240, 102_400, 1_048_576].iter() {
        group.bench_with_input(
            BenchmarkId::new("store_bytes", size),
            size,
            |b, &size| {
                let data = vec![0u8; size];
                b.to_async(&rt).iter(|| async {
                    client.store(black_box(data.clone())).await.unwrap()
                });
            },
        );
    }
...
fn benchmark_serialization(c: &mut Criterion) {
    ...
    group.bench_function("bincode_small", |b| {
        b.iter(|| {
            bincode::serialize(black_box(&small_msg)).unwrap()
        });
    });
    
    group.bench_function("bincode_large", |b| {
        b.iter(|| {
            bincode::serialize(black_box(&large_msg)).unwrap()
        });
    });
    ...
    group.bench_function("bincode_deserialize_small", |b| {
        b.iter(|| {
            bincode::deserialize::<NetworkMessage>(black_box(&small_bytes)).unwrap()
        });
    });
    
    group.bench_function("bincode_deserialize_large", |b| {
        b.iter(|| {
            bincode::deserialize::<NetworkMessage>(black_box(&large_bytes)).unwrap()
        });
    });
...
fn benchmark_routing(c: &mut Criterion) {
    ...
    group.bench_function("find_path", |b| {
        ...
        b.to_async(&rt).iter(|| async {
            router.find_path(black_box(&target)).await.unwrap()
        });
    });
...
fn benchmark_memory(c: &mut Criterion) {
    ...
    group.bench_function("hashmap_operations", |b| {
        ...
        b.iter(|| {
            for i in 0..100 {
                let key = NodeId { hash: [i as u8; 32] };
                map.insert(key, i);
            }
            map.clear();
        });
...
fn benchmark_crypto(c: &mut Criterion) {
    ...
    group.bench_function("ed25519_verify", |b| {
        b.iter(|| {
            identity.verify(black_box(message), black_box(&signature))
        });
    });
