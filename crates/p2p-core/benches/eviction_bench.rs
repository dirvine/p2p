...
use saorsa_core::adaptive::{
    EvictionStrategy, EvictionStrategyType, CacheState, AccessInfo, ContentHash,
};
...
fn bench_lru_eviction(c: &mut Criterion) {
    ...
    c.bench_function("lru_eviction_1000", |b| {
        ...
        b.iter(|| {
            black_box(strategy.select_victim(&cache_state, &cache_data))
        });
    });
}
...
criterion_group!(benches, bench_lru_eviction, bench_lfu_eviction, bench_adaptive_eviction);
