...
impl TrustProvider for MockTrustProvider {
    ...
    fn get_global_trust(&self) -> HashMap<NodeId, f64> { HashMap::new() }
    ...
    group.bench_function("graft_handling", |b| {
        ...
        b.iter(|| {
            runtime.block_on(async {
                ...
                black_box(gossipsub.handle_control_message(&peers[0], msg).await.unwrap());
            });
        });
    });
...
fn bench_adaptive_mesh_calculation(c: &mut Criterion) {
    ...
    group.bench_function("calculate_adaptive_size", |b| {
        ...
        runtime.block_on(async {
            gossipsub.set_topic_priority("critical", TopicPriority::Critical).await;
            gossipsub.set_topic_priority("normal", TopicPriority::Normal).await;
            gossipsub.set_topic_priority("low", TopicPriority::Low).await;
        });
