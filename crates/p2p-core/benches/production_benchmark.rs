...
use p2p_foundation::production::{ProductionConfig, ResourceManager, RateLimitConfig};
                ...
                let load_handles: Vec<_> = (0..5)
                    .map(|i| {
                        ...
                        tokio::spawn(async move {
                            ...
                            let _ = manager.check_rate_limit(&format!("peer_{}", i), "mcp").await;
                        })
                    })
                    .collect();
                ...
                for pattern in 0..10 {
                    match pattern % 3 {
                        ...
                        _ => {
                            ...
                            for i in 0..5 {
                                let _ = manager.check_rate_limit(&format!("burst_{}", i), "dht").await;
                            }
                        }
                    }
                }
