        ...
        group.bench_with_input(
            ...
            |b, &node_count| {
                b.to_async(&rt).iter(|| async move {
                    benchmark_multi_node_messaging(node_count).await
                });
            },
        );
    }
...
fn benchmark_storage_performance(c: &mut Criterion) {
    ...
    for data_size in [1024, 10240, 102400].iter() {
        group.bench_with_input(
            ...
            |b, &data_size| {
                b.to_async(&rt).iter(|| async move {
                    benchmark_storage_operations(data_size).await
                });
            },
        );
    }
...
async fn benchmark_multi_node_messaging(node_count: usize) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..10 {
        ...
        nodes[0].send_message(&recipient, message.into_bytes()).await?;
    }
...
async fn benchmark_encryption_operations(message_size: usize) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let encrypted = sender_node.encrypt_message(&message, &receiver_identity).await?;
    
    // Benchmark decryption  
    let _decrypted = receiver_node.decrypt_message(encrypted, &sender_identity).await?;
