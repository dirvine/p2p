...
impl PerformanceTestFramework {
    ...
    fn benchmark_baseline_throughput(&mut self) -> anyhow::Result<f64> {
        ...
        for i in 0..operations {
            ...
            self.rt.block_on(
                self.clients[0].store(&key, value)
            )?;
        }
        ...
        self.metrics.insert("baseline_throughput".to_string(), ops_per_sec);
        Ok(ops_per_sec)
    }
    ...
    fn benchmark_concurrent_operations(&mut self, concurrency: usize) -> anyhow::Result<f64> {
        ...
        let handles: Vec<_> = (0..concurrency).map(|thread_id| {
        ...
        self.metrics.insert(format\!("concurrent_throughput_{}", concurrency), ops_per_sec);
        Ok(ops_per_sec)
    }
    ...
    fn benchmark_memory_usage(&mut self) -> anyhow::Result<u64> {
        ...
        self.metrics.insert("memory_increase".to_string(), memory_increase as f64);
        Ok(memory_increase)
    }
