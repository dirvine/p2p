...
use saorsa_core::adaptive::{
    ...
    ContentHash, NodeId, Experience, ActionType,
};
...
fn benchmark_state_discretization(c: &mut Criterion) {
    ...
    group.bench_function("state_space_calculation", |b| {
        b.iter(|| {
            black_box(StateVector::state_space_size())
        })
    });
...
fn benchmark_action_selection(c: &mut Criterion) {
    ...
    for epsilon in [0.0, 0.1, 0.5, 1.0].iter() {
        ...
        rt.block_on(async {
            let state = StateVector::from_metrics(0.5, 10.0, 300, 1024);
            manager.update_q_value(&state, ActionType::Cache, 1.0, &state, true).await.unwrap();
            manager.update_q_value(&state, ActionType::Evict, 0.5, &state, true).await.unwrap();
            manager.update_q_value(&state, ActionType::DoNothing, 0.7, &state, true).await.unwrap();
        });
    ...
    for buffer_size in [100, 1000, 10000].iter() {
        ...
        rt.block_on(async {
            for i in 0..*buffer_size {
                ...
                manager.experience_buffer.write().await.push_back(experience);
            }
        });
        ...
        group.bench_with_input(
            ...
            |b, _| {
                b.iter(|| {
                    rt.block_on(async {
                        manager.learn_from_replay().await.unwrap()
                    })
                })
            },
        );
...
fn benchmark_cache_operations(c: &mut Criterion) {
    ...
    group.bench_function("get_current_state", |b| {
        ...
        b.iter(|| {
            rt.block_on(async {
                manager.get_current_state(black_box(&content_hash)).await.unwrap()
            })
        })
    });
            ...
            |b, _| {
                ...
                b.iter(|| {
                    rt.block_on(async {
                        ...
                        let actions = manager.get_available_actions(&content_hash, 10 * 1024)
                            .await
                            .unwrap();
