...
use crate::validation::{ValidationContext, validate_network_address, validate_file_path, validate_config_value};
...
impl Config {
    ...
    fn apply_env_overrides(&mut self) -> Result<()> {
        ...
        if let Ok(val) = env::var("SAORSA_MAX_CONNECTIONS") {
            self.network.max_connections = val.parse()
                .map_err(|_| P2PError::Config(ConfigError::InvalidValue { field: "max_connections".to_string().into(), reason: "Invalid value".to_string().into() }))?;
        }

        // Security overrides
        if let Ok(val) = env::var("SAORSA_RATE_LIMIT") {
            self.security.rate_limit = val.parse()
                .map_err(|_| P2PError::Config(ConfigError::InvalidValue { field: "rate_limit".to_string().into(), reason: "Invalid value".to_string().into() }))?;
        }
        if let Ok(val) = env::var("SAORSA_ENCRYPTION_ENABLED") {
            self.security.encryption_enabled = val.parse()
                .map_err(|_| P2PError::Config(ConfigError::InvalidValue { field: "encryption_enabled".to_string().into(), reason: "Invalid value".to_string().into() }))?;
        }
        ...
        if let Ok(val) = env::var("SAORSA_MCP_ENABLED") {
            self.mcp.enabled = val.parse()
                .map_err(|_| P2PError::Config(ConfigError::InvalidValue { field: "mcp_enabled".to_string().into(), reason: "Invalid value".to_string().into() }))?;
        }
        if let Ok(val) = env::var("SAORSA_MCP_PORT") {
            self.mcp.port = val.parse()
                .map_err(|_| P2PError::Config(ConfigError::InvalidValue { field: "mcp_port".to_string().into(), reason: "Invalid value".to_string().into() }))?;
        }
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if let Err(e) = validate_config_value(&self.network.max_connections.to_string(), Some(1_usize), Some(100000_usize)) {
        ...
        if let Err(e) = validate_config_value(&self.security.rate_limit.to_string(), Some(1_u32), Some(1000000_u32)) {
            errors.push(P2PError::Config(ConfigError::InvalidValue {
    ...
    fn validate_address(&self, addr: &str, field: &str) -> Result<()> {
        ...
        if let Ok(socket_addr) = SocketAddr::from_str(addr) {
            ...
            return validate_network_address(&socket_addr, &ctx)
    ...
    pub fn bootstrap_addrs(&self) -> Result<Vec<NetworkAddress>> {
        self.network.bootstrap_nodes
            .iter()
            .map(|addr| addr.parse::<NetworkAddress>()
                .map_err(|e| P2PError::Config(ConfigError::InvalidValue {
        ...
        SIZE_REGEX.with(|re| {
            if let Some(captures) = re.captures(size) {
                let value: f64 = captures.get(1)
                    ...
                    .ok_or_else(|| P2PError::Config(ConfigError::InvalidValue {
