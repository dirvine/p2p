...
impl IdentityKeyPair {
    ...
    pub fn needs_rotation(&self) -> bool {
        ...
        let rotation_time = self.expires_at.saturating_sub(KEY_ROTATION_WARNING.as_secs());
        now >= rotation_time
    }
...
impl Identity {
    ...
    pub fn from_key_pair(
        ...
        let metadata_size: usize = params.metadata.values()
            .map(|v| v.len())
            .sum();
...
impl IdentityManager {
    ...
    pub async fn create_identity(
        ...
        self.schedule_key_rotation_check(identity.id.clone()).await?;
    ...
    pub async fn verify_identity(&self, identity: &Identity) -> Result<IdentityVerification> {
        ...
        // Check metadata size
        let metadata_size: usize = identity.metadata.values()
            .map(|v| v.len())
            .sum();
        ...
        // Update stats
        {
            ...
            stats.avg_verification_time_us = 
                (stats.avg_verification_time_us + elapsed) / 2;
        }
