...
pub fn validate_peer_id(peer_id: &str) -> P2pResult<()> {
    ...
    if !length_valid {
        return Err(ValidationError::InvalidPeerId(
            format!("Length must be between {} and {} characters", MIN_PEER_ID_LENGTH, MAX_PEER_ID_LENGTH)
        ).into());
    }
...
pub fn validate_message_size(size: usize, max_size: usize) -> P2pResult<()> {
    if size > max_size {
        return Err(ValidationError::MessageTooLarge { size, limit: max_size }.into());
    }
    Ok(())
}
...
pub fn validate_file_path(path: &Path) -> P2pResult<()> {
    ...
    if path_str.len() > MAX_PATH_LENGTH {
        return Err(ValidationError::InvalidPath(
            format!("Path too long: {} > {}", path_str.len(), MAX_PATH_LENGTH)
        ).into());
    }
    
    // URL decode to catch encoded traversal attempts
    let decoded = path_str.replace("%2e", ".").replace("%2f", "/").replace("%5c", "\\");
    ...
    for component in path.components() {
        if let Some(name) = component.as_os_str().to_str() {
            if name.len() > MAX_FILE_NAME_LENGTH {
                return Err(ValidationError::InvalidPath(
                    format!("Component '{}' exceeds maximum length", name)
                ).into());
            }
            ...
            if name.contains('\0') {
                return Err(ValidationError::InvalidPath(
                    format!("Component '{}' contains invalid characters", name)
                ).into());
            }
        }
    }
...
pub fn validate_key_size(size: usize, expected: usize) -> P2pResult<()> {
    if size != expected {
        return Err(ValidationError::InvalidCryptoParam(
            format!("Invalid key size: expected {} bytes, got {}", expected, size)
        ).into());
    }
    Ok(())
}
...
pub fn validate_nonce_size(size: usize, expected: usize) -> P2pResult<()> {
    if size != expected {
        return Err(ValidationError::InvalidCryptoParam(
            format!("Invalid nonce size: expected {} bytes, got {}", expected, size)
        ).into());
    }
    Ok(())
}
        ...
        for (key, value) in &self.params {
            ...
            for pattern in &sql_patterns {
                if lower_value.contains(pattern) {
                    return Err(ValidationError::InvalidFormat(
                        format!("Suspicious parameter value: potential SQL injection")
                    ).into());
                }
            }
    ...
    T: PartialOrd + std::fmt::Display,
{
    ...
    if let Some(min_val) = min {
        if parsed < min_val {
            return Err(ValidationError::InvalidFormat(
                format!("Value {} is less than minimum {}", parsed, min_val)
            ).into());
        }
    }
    ...
    if let Some(max_val) = max {
        if parsed > max_val {
            return Err(ValidationError::InvalidFormat(
                format!("Value {} is greater than maximum {}", parsed, max_val)
            ).into());
        }
    }
...
mod tests {
    ...
    fn test_sanitization() {
        assert_eq!(
            sanitize_string("hello world!", 20),
            "helloworld"
        );
        
        assert_eq!(
            sanitize_string("test@#$%123", 20),
            "test123"
        );
