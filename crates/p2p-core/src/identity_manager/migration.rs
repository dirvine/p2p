...
impl IdentityMigrator {
    ...
    pub async fn migrate(
        ...
        for entry in entries {
            let entry = entry.map_err(|e| P2PError::Storage(StorageError::Database(
            ...
            if path.extension().and_then(|s| s.to_str()) == Some("json") {
                ...
                match self.migrate_identity_file(&path, &manager, storage_password, device_password).await {
                    Ok(true) => {
                        status.migrated += 1;
    ...
    async fn migrate_identity_file(
        ...
        let sync_data = serde_json::to_vec(&sync_package)
            .map_err(|e| P2PError::Serialization(e.to_string().into()))?;
        fs::write(&sync_backup_path, sync_data)
            .map_err(|e| P2PError::Storage(StorageError::Database(
    ...
    pub async fn verify_migration(&self) -> Result<bool> {
        ...
        info!("Verification - Source: {}, Target: {}, Backup: {}", 
...
mod tests {
    ...
    async fn test_migration_tool() {
        ...
        // Create migrator
        let migrator = IdentityMigrator::new(
            source_dir.path(),
            target_dir.path(),
            backup_dir.path(),
        ).unwrap();
        ...
        let status = migrator.migrate(&storage_password, &device_password).await.unwrap();
