...
pub enum SystemMessage {
    UserJoined(UserId),
    UserLeft(UserId),
    UserInvited { inviter: UserId, invitee: UserId },
    ChannelRenamed { old_name: String, new_name: String },
    ChannelDescriptionChanged,
    CallStarted { call_id: String },
    CallEnded { call_id: String, duration_seconds: u64 },
}
...
pub enum AttachmentMetadata {
    ...
    Video { duration_seconds: u64, width: u32, height: u32 },
...
impl ChatManager {
    ...
    pub async fn send_message(
        ...
        if !channel.members.iter().any(|m| m.user_id == self.identity.base_identity.user_id) {
            return Err(ChatError::PermissionDenied("Not a member of channel".to_string()));
        }
    ...
    async fn add_public_channel(&mut self, channel_id: &ChannelId, name: &str) -> Result<()> {
        ...
        let mut public_channels: HashMap<String, String> = self.storage
            .get_public(&key).await
            .unwrap_or_default();
        
        public_channels.insert(channel_id.0.clone(), name.to_string());
        
        self.storage.store_public(
            &key,
            &public_channels,
            ttl::PROFILE,
        ).await?;
