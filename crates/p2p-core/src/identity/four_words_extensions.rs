...
use crate::{Result, P2PError, error::IdentityError};
...
impl FourWordAddress {
    ...
    pub fn from_node_id(node_id: &NodeId) -> Result<Self> {
        ...
        WordEncoder::encode(node_id.to_bytes())
            .map_err(|e| P2PError::Identity(IdentityError::InvalidFourWordAddress(
...
mod tests {
    ...
    fn test_four_word_address_format() {
        ...
        let address = FourWordAddress::from_node_id(&node_id).expect("Should create address from node ID");
