...
impl IdentityCliHandler {
    ...
    pub async fn execute(&self, command: Commands) -> Result<String> {
        match command {
            Commands::Generate { difficulty, output, seed } => {
                self.handle_generate(difficulty, output, seed).await
            },
            Commands::Show { path } => {
                self.handle_show(path).await
            },
            Commands::Verify { path } => {
                self.handle_verify(path).await
            },
            Commands::Export { path, output, format } => {
                self.handle_export(path, output, format).await
            },
            Commands::Sign { identity, message, output } => {
                self.handle_sign(identity, message, output).await
            },
        }
    }
    
    async fn handle_generate(&self, difficulty: Option<u32>, output: Option<PathBuf>, seed: Option<String>) -> Result<String> {
        let difficulty = difficulty.unwrap_or(16);
        let output_path = output.or_else(|| self.default_path.clone())
            .ok_or_else(|| crate::P2PError::Identity(crate::error::IdentityError::InvalidFormat(
    ...
    async fn handle_show(&self, path: Option<PathBuf>) -> Result<String> {
        let path = path.or_else(|| self.default_path.clone())
            .ok_or_else(|| crate::P2PError::Identity(crate::error::IdentityError::InvalidFormat(
    ...
    async fn handle_export(&self, path: Option<PathBuf>, output: PathBuf, format: String) -> Result<String> {
        let path = path.or_else(|| self.default_path.clone())
            .ok_or_else(|| crate::P2PError::Identity(crate::error::IdentityError::InvalidFormat(
                "No identity found".into()
            )))?;
        ...
        match format.as_str() {
            ...
            _ => Err(crate::P2PError::Identity(crate::error::IdentityError::InvalidFormat(
                format!("Unsupported format: {}", format).into()
            )))
        }
    }
    
    async fn handle_sign(&self, identity_path: Option<PathBuf>, message: String, output: Option<PathBuf>) -> Result<String> {
        let path = identity_path.or_else(|| self.default_path.clone())
            .ok_or_else(|| crate::P2PError::Identity(crate::error::IdentityError::InvalidFormat(
                "No identity found".into()
            )))?;
        ...
        let message_bytes = if message.starts_with("@") {
            // File path
            tokio::fs::read(&message[1..]).await
                .map_err(|e| crate::P2PError::Identity(crate::error::IdentityError::InvalidFormat(
        ...
        if let Some(output_path) = output {
            tokio::fs::write(&output_path, &sig_hex).await
                .map_err(|e| crate::P2PError::Identity(crate::error::IdentityError::InvalidFormat(
...
impl IdentityCommand {
        ...
        T: Into<std::ffi::OsString> + Clone,
    {
        ...
        let args: Vec<String> = iter.into_iter()
        ...
        match args[1].as_str() {
            "generate" => {
                ...
                Ok(IdentityCommand { command: Commands::Generate { difficulty, output: None, seed: None } })
            },
            "show" => {
                let mut path = None;
                let mut i = 2;
                ...
                Ok(IdentityCommand { command: Commands::Show { path } })
            },
