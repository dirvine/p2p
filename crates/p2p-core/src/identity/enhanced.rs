...
impl EnhancedIdentityManager {
    ...
    pub async fn create_enhanced_identity(
        ...
        let peer_id = crate::quantum_crypto::types::PeerId(
            base_identity.user_id.as_bytes().to_vec()
        );
    ...
    pub fn check_permission(
        ...
        let membership = user.organizations.iter()
            .find(|m| &m.org_id == org_id)
            .ok_or_else(|| EnhancedIdentityError::PermissionDenied(
                "User not in organization".to_string()
            ))?;
        ...
        let allowed = match (&membership.role, &permission) {
            ...
            (OrganizationRole::Member, perm) => matches!(perm,
            ...
            (OrganizationRole::Guest, perm) => matches!(perm,
                Permission::ChatVoiceVideo
            ),
        };
