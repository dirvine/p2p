...
impl ThresholdGroup {
    ...
    pub fn check_permission(
        ...
        let participant = self.active_participants
            .iter()
        ...
        match (&participant.role, permission) {
            (ParticipantRole::Leader { permissions }, Permission::AddParticipant) => {
                if permissions.can_add_participants {
                    ...
                    Err(ThresholdError::Unauthorized("Cannot add participants".to_string()))
                }
            }
            (ParticipantRole::Leader { permissions }, Permission::RemoveParticipant) => {
                if permissions.can_remove_participants {
                    ...
                    Err(ThresholdError::Unauthorized("Cannot remove participants".to_string()))
                }
            }
            (ParticipantRole::Leader { permissions }, Permission::UpdateThreshold) => {
                if permissions.can_update_threshold {
                    ...
                    Err(ThresholdError::Unauthorized("Cannot update threshold".to_string()))
                }
            }
            ...
            (ParticipantRole::Observer, _) => {
                Err(ThresholdError::Unauthorized("Observers have read-only access".to_string()))
            }
            _ => Err(ThresholdError::Unauthorized("Permission denied".to_string())),
        }
    }
    ...
    pub fn add_pending_participant(&mut self, participant: ParticipantInfo) -> Result<()> {
        ...
        if self.active_participants.iter().any(|p| p.participant_id == participant.participant_id) {
            return Err(ThresholdError::InvalidParameters(
                "Participant already exists".to_string()
            ));
        }
        
        if self.pending_participants.iter().any(|p| p.participant_id == participant.participant_id) {
            return Err(ThresholdError::InvalidParameters(
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        let has_leader = self.active_participants
            .iter()
