...
impl EigenTrustEngine {
    ...
    pub async fn update_node_stats(
        &self,
        node_id: &NodeId,
        stats_update: NodeStatisticsUpdate,
    ) {
        ...
        match stats_update {
            ...
            NodeStatisticsUpdate::BandwidthContributed(gb) => node_stats.bandwidth_contributed += gb,
            NodeStatisticsUpdate::ComputeContributed(cycles) => node_stats.compute_contributed += cycles,
        }
    }
    ...
    pub async fn compute_global_trust(&self) -> HashMap<NodeId, f64> {
        ...
        for _ in 0..50 {
            ...
            for node in &nodes {
                ...
                for other in &nodes {
                    if let Some(local_trust_val) = self.get_normalized_trust(&local_trust, other, node) {
                        trust_sum += local_trust_val * 
    ...
    fn update_metrics(&mut self, path: &[NodeId], success: bool) {
        ...
        if path.len() >= 2 {
            for window in path.windows(2) {
                self.trust_engine.update_trust(&window[0], &window[1], success);
            }
        }
    }
}
...
impl TrustProvider for MockTrustProvider {
    fn get_trust(&self, node: &NodeId) -> f64 {
        self.trust_scores.blocking_read().get(node).copied().unwrap_or(0.5)
    }
...
mod tests {
    ...
    async fn test_eigentrust_basic() {
        ...
        let pre_trusted = HashSet::from([
            NodeId::from_bytes(hash_pre),
        ]);
        ...
        engine.update_local_trust(pre_trusted_node, &node1, true).await;
    ...
    async fn test_multi_factor_trust() {
        ...
        engine.update_node_stats(&node, NodeStatisticsUpdate::CorrectResponse).await;
        engine.update_node_stats(&node, NodeStatisticsUpdate::CorrectResponse).await;
        engine.update_node_stats(&node, NodeStatisticsUpdate::FailedResponse).await;
        engine.update_node_stats(&node, NodeStatisticsUpdate::StorageContributed(100)).await;
    ...
    async fn test_trust_based_routing() {
        ...
        let engine = Arc::new(EigenTrustEngine::new(HashSet::from([pre_trusted_id.clone()])));
        ...
        engine.update_local_trust(&pre_trusted_id, &local_id, true).await;
