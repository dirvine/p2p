...
impl MultiArmedBandit {
    ...
    pub async fn select_route(
        ...
        if should_explore {
            ...
            // Random exploration
            let strategy = available_strategies[rand::random::<usize>() % available_strategies.len()];
            ...
            let distribution = BetaDistribution::new(stats.alpha, stats.beta)
                .map_err(|e| P2PError::Config(ConfigError::ValidationFailed(format!("Beta distribution error: {}", e).into())))?;
        ...
        let route_id = best_route.ok_or_else(|| 
            P2PError::Config(ConfigError::ValidationFailed("No routes available".to_string().into()))
        )?;

        let distribution = BetaDistribution::new(best_stats.alpha, best_stats.beta)
            .map_err(|e| P2PError::Config(ConfigError::ValidationFailed(format!("Beta distribution error: {}", e).into())))?;
    ...
    pub async fn update_route(
        ...
        stats.avg_latency_ms = (1.0 - alpha) * stats.avg_latency_ms + alpha * outcome.latency_ms as f64;
        ...
        if last_persist.elapsed() > self.config.persist_interval {
            if let Some(ref path) = self.config.storage_path {
                ...
                tokio::spawn(async move {
                    if let Err(e) = Self::persist_statistics_static(&path_clone, &statistics_clone, &metrics_clone).await {
    ...
    async fn load_statistics(&mut self, path: &Path) -> Result<()> {
        ...
        let data = fs::read_to_string(&stats_path).await
            .map_err(|e| P2PError::Io(e))?;
        
        let (statistics, metrics): (HashMap<(RouteId, ContentType), RouteStatistics>, MABMetrics) = 
            serde_json::from_str(&data)
                .map_err(|e| P2PError::Storage(crate::error::StorageError::Database(format!("Failed to deserialize statistics: {}", e).into())))?;
        ...
        *self.metrics.write().await = metrics;
        ...
        tracing::info!("Loaded {} route statistics from disk", self.statistics.read().await.len());
        Ok(())
    }
    ...
    async fn persist_statistics_static(
        ...
        let data = serde_json::to_string_pretty(&(statistics, metrics))
            .map_err(|e| P2PError::Storage(crate::error::StorageError::Database(format!("Failed to serialize statistics: {}", e).into())))?;
        
        fs::write(&stats_path, data).await
            .map_err(|e| P2PError::Io(e))?;
