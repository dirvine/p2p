...
impl AdaptiveRouter {
    ...
    pub async fn route(&self, target: &NodeId, content_type: ContentType) -> Result<Vec<NodeId>> {
        ...
        let strategy_choice = self.bandit.read().await.select_strategy(content_type).await
        ...
        self.bandit.write().await.update(content_type, strategy_choice, success, latency as u64).await
            .unwrap_or(());
...
mod tests {
    ...
    fn test_thompson_sampling() {
        ...
        let strategy = rt.block_on(bandit.select_strategy(ContentType::DHTLookup)).unwrap_or(StrategyChoice::Kademlia);
        assert!(matches!(
            strategy,
        ...
        // Test update
        rt.block_on(bandit.update(ContentType::DHTLookup, strategy, true, 100)).unwrap();
    }
    
    #[test]
    fn test_hyperbolic_distance() {
        let a = HyperbolicCoordinate { r: 0.0, theta: 0.0 };
        let b = HyperbolicCoordinate { r: 0.5, theta: std::f64::consts::PI };
