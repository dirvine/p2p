...
impl SecurityManager {
    ...
    pub async fn check_rate_limit(&self, node_id: &NodeId, ip: Option<IpAddr>) -> Result<(), SecurityError> {
        // Check node rate limit
        if !self.rate_limiter.check_node_rate(node_id).await {
            self.blacklist.record_violation(node_id, BlacklistReason::RateLimitViolation).await;
            self.auditor.log_event(SecurityEvent::RateLimitExceeded,
                Some(node_id.clone()),
    ...
    pub async fn detect_eclipse_attack(&self, routing_table: &[NodeId]) -> Result<(), SecurityError> {
        let diversity_score = self.eclipse_detector.calculate_diversity_score(routing_table).await;
        ...
        if self.eclipse_detector.detect_suspicious_patterns(routing_table).await {
            self.auditor.log_event(SecurityEvent::EclipseAttackDetected,
                None,
    ...
    pub async fn verify_message_integrity(&self, message: &[u8], hash: &[u8], signature: Option<&[u8]>) -> Result<(), SecurityError> {
        // Check message size
    ...
    pub async fn blacklist_node(&self, node_id: NodeId, reason: BlacklistReason) {
        self.blacklist.add_entry(node_id.clone(), reason.clone()).await;
...
impl BlacklistManager {
    ...
    pub async fn add_entry(&self, node_id: NodeId, reason: BlacklistReason) {
        ...
        if blacklist.len() >= self.config.max_entries {
            // Remove oldest entry
            if let Some(oldest) = blacklist.iter()
        ...
        blacklist.insert(node_id.clone(), BlacklistEntry {
            node_id,
            reason,
...
impl SecurityAuditor {
    ...
    /// Log security event
    pub async fn log_event(&self, event_type: SecurityEvent, node_id: Option<NodeId>, details: String, severity: Severity) {
    ...
    /// Get audit entries
    pub async fn get_entries(&self, since: Option<SystemTime>, severity_filter: Option<Severity>) -> Vec<AuditEntry> {
    ...
    pub async fn export_report(&self) -> AuditReport {
        ...
        AuditReport {
            ...
            recent_critical_events: entries.iter()
...
mod tests {
    ...
    async fn test_blacklist_management() {
        ...
        // Add to blacklist
        blacklist.add_entry(node_id.clone(), BlacklistReason::MaliciousBehavior("Test".to_string())).await;
    ...
    async fn test_security_manager_integration() {
        ...
        // Blacklist the node
        manager.blacklist_node(node.id.clone(), BlacklistReason::Manual("Test".to_string())).await;
