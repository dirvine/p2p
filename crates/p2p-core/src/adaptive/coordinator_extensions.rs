...
impl ContentStoreExtensions for ContentStore {
    async fn store_with_strategy(&self, data: &[u8], _strategy: StorageStrategy) -> Result<()> {
        ...
        let _ = self.store(data.to_vec(), metadata).await
...
pub trait MultiArmedBanditExtensions {
    ...
    async fn update_strategy_performance(&self, strategy: RetrievalStrategy, success: bool, latency: Duration);
...
impl MultiArmedBanditExtensions for MultiArmedBandit {
    ...
    async fn update_strategy_performance(&self, _strategy: RetrievalStrategy, _success: bool, _latency: Duration) {
...
pub trait ReplicationManagerExtensions {
    ...
    async fn replicate(&self, hash: &ContentHash, data: Vec<u8>, strategy: ReplicationStrategy) -> Result<()>;
    async fn start_monitoring(&self);
...
impl ReplicationManagerExtensions for ReplicationManager {
    ...
    async fn replicate(&self, _hash: &ContentHash, _data: Vec<u8>, _strategy: ReplicationStrategy) -> Result<()> {
