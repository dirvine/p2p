...
impl ThompsonSampling {
    ...
    pub async fn update(&self, 
        ...
        *current_rate = 0.9 * (*current_rate) + 0.1 * success_rate;
        ...
        let current_latency = metrics.strategy_latencies.entry(strategy).or_insert(latency_ms as f64);
...
impl QLearnCacheManager {
    ...
    fn get_current_state(&self, _content_hash: &ContentHash) -> CacheState {
        let utilization = (self.current_size.load(std::sync::atomic::Ordering::Relaxed) * 10 / self.capacity) as u8;
    ...
    pub async fn get_current_state_async(&self, content_hash: &ContentHash) -> CacheState {
        ...
        let (request_rate_bucket, content_popularity, size_bucket) = if let Some(stat) = stats.get(content_hash) {
    ...
    pub async fn insert(&self, hash: ContentHash, data: Vec<u8>) -> bool {
        ...
        cache.insert(hash, CachedContent {
            data,
            access_count: 0,
    ...
    async fn evict_one(&self) -> bool {
        ...
        if let Some(key) = oldest {
            if let Some(value) = cache.remove(&key) {
                self.current_size.fetch_sub(value.data.len(), std::sync::atomic::Ordering::Relaxed);
                return true;
            }
        }
    ...
    pub fn calculate_reward(&self, action: CacheAction, hit: bool, bandwidth_cost: u64) -> f64 {
        ...
        let storage_cost = self.current_size.load(std::sync::atomic::Ordering::Relaxed) as f64 / self.capacity as f64;
    ...
    pub async fn execute_action(&self, hash: &ContentHash, action: CacheAction, data: Option<Vec<u8>>) -> Result<()> {
        match action {
            CacheAction::Cache => {
...
impl ChurnPredictor {
    ...
    pub async fn update_node_features(&self, node_id: &NodeId, features: Vec<f64>) -> anyhow::Result<()> {
        if features.len() != 10 {
            return Err(anyhow::anyhow!("Expected 10 features, got {}", features.len()));
        }
        ...
        entry.snapshots.push((std::time::Instant::now(), node_features));
    ...
    pub async fn extract_features(&self, node_id: &NodeId) -> Option<NodeFeatures> {
        ...
        let completed_sessions: Vec<_> = node_history.sessions.iter()
            .filter_map(|(start, end)| end.as_ref().map(|e| e.duration_since(*start).as_secs() as f64))
            .collect();
        ...
        let recent_disconnections = node_history.sessions.iter()
            .filter(|(start, end)| end.is_some() && *start > one_week_ago)
            .count() as f64;
        
        // Get latest snapshot for other features
        let latest_snapshot = node_history.snapshots.last()
    ...
    async fn analyze_patterns(&self, features: &NodeFeatures) -> HashMap<String, f64> {
        ...
        // Behavior patterns
        patterns.insert("short_session".to_string(), 
            if features.online_duration < 1800.0 { 1.0 } else { 0.0 });
        patterns.insert("unstable".to_string(), 
            if features.recent_disconnections > 5.0 { 1.0 } else { 0.0 });
        patterns.insert("low_contribution".to_string(), 
            if features.resource_contribution < 0.3 { 1.0 } else { 0.0 });
        patterns.insert("slow_response".to_string(), 
            if features.avg_response_time > 500.0 { 1.0 } else { 0.0 });
        ...
        patterns.insert("high_risk".to_string(), if risk_score > 0.6 { 1.0 } else { 0.0 });
    ...
    async fn apply_model(
        ...
        let probabilities: Vec<f64> = base_scores.iter()
    ...
    pub async fn update_node_behavior(
        ...
        let mut history = self.feature_history.write().await;
        let node_history = history.entry(node_id.clone()).or_insert_with(|| {
            FeatureHistory {
        ...
        // Add snapshot
        node_history.snapshots.push((std::time::Instant::now(), features));
        
        // Keep only recent snapshots (last 24 hours)
        let cutoff = std::time::Instant::now() - std::time::Duration::from_secs(24 * 3600);
        node_history.snapshots.retain(|(timestamp, _)| *timestamp > cutoff);
    ...
    pub async fn record_node_event(&self, node_id: &NodeId, event: NodeEvent) -> Result<()> {
        ...
        match event {
            NodeEvent::Connected => {
                // Start new session
                node_history.sessions.push((std::time::Instant::now(), None));
            }
    ...
    async fn update_model(&self) -> anyhow::Result<()> {
        ...
        for example in batch {
            ...
            for (i, &feature_value) in feature_vec.iter().enumerate() {
                if i < model.feature_weights.len() {
                    for (j, &error) in errors.iter().enumerate() {
                        model.feature_weights[i] += learning_rate * error * feature_value * model.time_decay[j];
                    }
                }
            }
            ...
            for (pattern, &value) in &patterns {
                ...
                model.pattern_weights.entry(pattern.clone())
...
mod tests {
    ...
    async fn test_reset_strategy() {
        ...
        // Reset it
        ts.reset_strategy(ContentType::RealtimeMessage, StrategyChoice::SOMRegion).await;
    ...
    async fn test_q_value_update() {
        ...
        manager.update_q_value(state, CacheAction::Cache, 1.0, next_state).await;
    ...
    async fn test_reward_calculation() {
        ...
        let evict_reward = manager.calculate_reward(CacheAction::Evict(EvictionPolicy::LRU), false, 0);
        assert!(evict_reward >= 0.0); // Should be slightly positive for evicting unused content
        
        let evict_penalty = manager.calculate_reward(CacheAction::Evict(EvictionPolicy::LRU), true, 0);
