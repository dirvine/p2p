...
impl ChurnHandler {
    ...
    async fn handle_imminent_departure(&self, node_id: &NodeId) -> Result<()> {
        ...
        self.node_monitor.update_node_state(node_id, NodeState::Departing).await;
        ...
        for content_hash in stored_content {
            self.recovery_manager.increase_replication(
                &content_hash,
    ...
    async fn handle_node_failure(&self, node_id: &NodeId) -> Result<()> {
        ...
        self.node_monitor.update_node_state(node_id, NodeState::Failed).await;
        ...
        for content_hash in lost_content {
            self.recovery_manager.queue_recovery(
                content_hash,
                vec![node_id.clone()],
    ...
    async fn handle_high_churn(&self) -> Result<()> {
        ...
        self.replication_manager.increase_global_replication(1.5).await;
    ...
    async fn identify_lost_content(&self, failed_node: &NodeId) -> Result<Vec<ContentHash>> {
        ...
        for content_hash in all_content {
            let remaining_replicas = self.recovery_manager
...
impl RecoveryManager {
    ...
    pub async fn increase_replication(
        ...
        self.queue_recovery(content_hash.clone(), vec![], priority).await
    }
    ...
    pub async fn get_remaining_replicas(
        ...
        if let Some(tracker) = self.content_tracker.read().await.get(content_hash) {
            let remaining = tracker.storing_nodes
                .iter()
...
mod tests {
    ...
    async fn test_proactive_replication() {
        ...
        handler.predictor.update_node_features(&node_id, vec![
    ...
    async fn test_churn_rate_calculation() {
        ...
        for i in 0..3 {
            let node_id = NodeId { hash: [i; 32] };
            handler.node_monitor.update_node_state(&node_id, NodeState::Failed).await;
        }
