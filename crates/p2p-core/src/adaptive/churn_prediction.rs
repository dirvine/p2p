...
impl LSTMChurnPredictor {
    ...
    pub async fn extract_features(
        ...
        let recent_stats = churn_detector.get_recent_stats(Duration::from_secs(3600)).await;
        ...
        ChurnFeatures {
            ...
            node_uptime: recent_stats.get_node_uptime(&NodeId::from_bytes(*node_id.node_id().to_bytes())),
            join_leave_ratio,
        }
    }
    ...
    fn calculate_confidence(&self, features: &ChurnFeatures) -> f64 {
        ...
        let valid_history = features.churn_history.iter()
            .filter(|&&r| r > 0.0)
            .count();
    ...
    fn backward_pass(&mut self, experience: &Experience) -> Result<()> {
        ...
        let prediction = tokio::runtime::Handle::current()
            .block_on(self.predict(features))?;
        
        // Calculate loss
        let pred_vec = vec![prediction.hour_1, prediction.hour_6, prediction.hour_24];
        let _losses: Vec<f64> = pred_vec.iter()
    ...
    pub async fn load(path: &std::path::Path) -> Result<Self> {
        ...
        let layers = model_data.layers.into_iter()
            .map(|d| d.into())
            .collect();
    ...
    pub async fn get_replication_recommendations(
        ...
        for node in nodes {
            let features = self.extract_features(node, churn_detector, network_size).await;
        ...
        recommendations.sort_by(|a, b| {
            b.1.hour_1.partial_cmp(&a.1.hour_1).unwrap_or(std::cmp::Ordering::Equal)
        });
