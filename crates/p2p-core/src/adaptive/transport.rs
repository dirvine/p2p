...
impl TransportListener for TcpTransportListener {
    async fn accept(&self) -> Result<(Box<dyn TransportConnection>, SocketAddr)> {
        ...
        Ok((Box::new(TcpTransportConnection { stream, info }), remote_addr))
    }
...
impl TransportManager {
    ...
    pub fn new() -> Self {
        ...
        transports.insert(TransportProtocol::Tcp, Arc::new(TcpTransport::new()) as Arc<dyn Transport>);
        transports.insert(TransportProtocol::Quic, Arc::new(QuicTransport::new()) as Arc<dyn Transport>);
