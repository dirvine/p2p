...
use super::q_learning_cache::{StateVector, CacheAction, AccessInfo};
...
impl EvictionStrategy for LRUStrategy {
    ...
    fn on_access(&mut self, content_hash: &ContentHash) {
        ...
        self.position_map.insert(content_hash.clone(), self.access_order.len() - 1);
    }
...
impl EvictionStrategy for LFUStrategy {
    fn select_victim(
        ...
        access_info.keys()
            .min_by_key(|&hash| {
                self.frequency_map.get(hash).unwrap_or(&0)
            })
            .cloned()
    }
...
impl AdaptiveStrategy {
    ...
    async fn calculate_retention_value(
        ...
        let state = StateVector {
            utilization_bucket: ((cache_state.current_size as f64 / cache_state.max_size as f64) * 10.0) as u8,
        ...
        let keep_value = q_table.get(&(state, CacheAction::DoNothing))
            .map(|qv| qv.value)
            .unwrap_or(0.0);
        let evict_value = q_table.get(&(state, CacheAction::Evict(content_hash.clone())))
            .map(|qv| qv.value)
            .unwrap_or(0.0);
    ...
    fn recency_bucket(&self, last_access_secs: u64) -> u8 {
        ...
        match age_secs {
            0..=60 => 0,           // Last minute
            61..=3600 => 1,        // Last hour
            3601..=86400 => 2,     // Last day
            86401..=604800 => 3,   // Last week
            _ => 4,                // Older
        }
    }
...
mod tests {
    ...
    async fn test_adaptive_eviction_uses_q_values() {
        ...
        q_table.write().await.insert(
            ...
            QValue { value: 10.0, updates: 1 },
        );
        q_table.write().await.insert(
            (state1, CacheAction::Evict(hash1.clone())),
            QValue { value: 5.0, updates: 1 },
        );
        
        // Make content2 less valuable
        q_table.write().await.insert(
            (state1, CacheAction::DoNothing),
            QValue { value: 3.0, updates: 1 },
        );
        q_table.write().await.insert(
            (state1, CacheAction::Evict(hash2.clone())),
            QValue { value: 8.0, updates: 1 },
        );
