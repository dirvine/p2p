...
impl ReplicationManager {
    ...
    pub async fn calculate_replication_factor(&self, content_hash: &ContentHash) -> u32 {
        ...
        factor.max(self.config.min_replicas).min(self.config.max_replicas)
    }
    ...
    pub async fn maintain_replications(&self) -> Result<()> {
        ...
        for (content_hash, mut replica_info) in content_to_check {
            ...
            if !at_risk_nodes.is_empty() {
                ...
                let replacement_nodes = self.select_replication_nodes(
                    &content_hash,
                    at_risk_nodes.len(),
            ...
            self.replica_map.write().await.insert(content_hash, replica_info);
        }
    ...
    pub async fn handle_node_departure(&self, departed_node: &NodeId) -> Result<()> {
        ...
        for (content_hash, mut replica_info) in affected_content {
            ...
            if replica_info.replication_factor < replica_info.target_factor {
                let needed = (replica_info.target_factor - replica_info.replication_factor) as usize;
                let new_nodes = self.select_replication_nodes(
                    &content_hash,
                    needed,
                    &replica_info.storing_nodes,
...
mod tests {
    ...
    async fn test_node_selection_excludes_nodes() {
        ...
        let nodes = manager.select_replication_nodes(&content_hash, 5, &exclude).await.unwrap();
    ...
    async fn test_replication_tracking() {
        ...
        // Perform replication
        let replica_info = manager.replicate_content(&content_hash, content, metadata).await.unwrap();
    ...
    async fn test_proactive_replication() {
        ...
        // Add some nodes
        for i in 0..3 {
            replica_info.storing_nodes.insert(NodeId { hash: [i as u8; 32] });
        }
        
        manager.replica_map.write().await.insert(content_hash, replica_info);
        
        // Run maintenance
        manager.maintain_replications().await.unwrap();
        
        // Check that maintenance was performed
        let updated = manager.replica_map.read().await.get(&content_hash).unwrap().clone();
    ...
    async fn test_node_departure_handling() {
        ...
        manager.replica_map.write().await.insert(content_hash.clone(), replica_info);
