...
pub trait AdaptiveP2PClient: Send + Sync {
    ...
    async fn connect(config: ClientConfig) -> Result<Self> where Self: Sized;
...
impl AdaptiveP2PClient for Client {
    ...
    async fn delete(&self, hash: &ContentHash) -> Result<()> {
        ...
        self.components.storage.delete(hash).await
    ...
    async fn subscribe(&self, topic: &str) -> Result<MessageStream> {
        ...
        // Subscribe to topic
        self.components.gossip.subscribe(topic).await
