...
pub enum DhtNetworkResult {
    ...
    GetSuccess { key: Key, value: Vec<u8>, source: PeerId },
    /// GET operation found no value
    GetNotFound { key: Key },
    /// Nodes found for FIND_NODE
    NodesFound { key: Key, nodes: Vec<SerializableDHTNode> },
    /// Value found for FIND_VALUE
    ValueFound { key: Key, value: Vec<u8>, source: PeerId },
    /// Ping response
    PongReceived { responder: PeerId, latency: Duration },
    /// Join confirmation
    JoinSuccess { assigned_key: Key, bootstrap_peers: usize },
    /// Leave confirmation
    LeaveSuccess,
    /// Operation failed
...
pub enum DhtNetworkEvent {
    ...
    OperationCompleted { operation: String, success: bool, duration: Duration },
    /// DHT network status changed
    NetworkStatusChanged { connected_peers: usize, routing_table_size: usize },
    /// Error occurred
    Error { error: String },
}
...
impl DhtNetworkManager {
    ...
    pub async fn new(config: DhtNetworkConfig) -> Result<Self> {
        info!("Creating DHT Network Manager for peer: {}", config.local_peer_id);
    ...
    pub async fn put(&self, key: Key, value: Vec<u8>) -> Result<DhtNetworkResult> {
        info!("Putting value for key: {} ({} bytes)", key.to_hex(), value.len());
        ...
        let closest_nodes = self.find_closest_nodes(&key, self.config.replication_factor).await?;
        ...
        for node in &closest_nodes {
            match self.send_dht_request(&node.peer_id, operation.clone()).await {
        ...
        info!("PUT operation completed: key={}, replicated_to={}/{}", 
              key.to_hex(), replicated_count, closest_nodes.len() + 1);
        
        Ok(DhtNetworkResult::PutSuccess { key, replicated_to: replicated_count })
    }
        ...
        for node in &closest_nodes {
            match self.send_dht_request(&node.peer_id, operation.clone()).await {
                ...
                Ok(DhtNetworkResult::GetNotFound { .. }) => {
                    debug!("Peer {} does not have value for key {}", node.peer_id, key.to_hex());
                }
                Ok(result) => {
                    warn!("Unexpected result from peer {}: {:?}", node.peer_id, result);
                }
    ...
    pub async fn find_node(&self, key: &Key) -> Result<DhtNetworkResult> {
        ...
        let closest_nodes = self.find_closest_nodes(key, self.config.replication_factor * 2).await?;
        let serializable_nodes: Vec<SerializableDHTNode> = closest_nodes
            .into_iter()
            .map(|node| node.to_serializable())
            .collect();
        
        info!("Found {} nodes closest to key: {}", serializable_nodes.len(), key.to_hex());
    ...
    async fn join_network(&self) -> Result<()> {
        ...
        for bootstrap_node in &self.config.bootstrap_nodes {
            match self.send_dht_request(&bootstrap_node.peer_id, join_operation.clone()).await {
                Ok(DhtNetworkResult::JoinSuccess { .. }) => {
                    bootstrap_peers += 1;
                    info!("Successfully joined via bootstrap node: {}", bootstrap_node.peer_id);
                }
                Ok(result) => {
                    warn!("Unexpected join result from {}: {:?}", bootstrap_node.peer_id, result);
                }
                Err(e) => {
                    warn!("Failed to join via bootstrap node {}: {}", bootstrap_node.peer_id, e);
                }
            }
        }
    ...
    async fn leave_network(&self) -> Result<()> {
        ...
        for peer_id in connected_peers {
            match self.send_dht_request(&peer_id, leave_operation.clone()).await {
                Ok(_) => {
    ...
    async fn find_closest_nodes(&self, key: &Key, count: usize) -> Result<Vec<DHTNode>> {
        ...
        for peer_id in known_peers.iter().take(3) { // Query up to 3 peers
    ...
    async fn send_dht_request(&self, peer_id: &PeerId, operation: DhtNetworkOperation) -> Result<DhtNetworkResult> {
        ...
        self.active_operations.write().await.insert(message_id.clone(), operation_context);
        
        // Send message via network layer
        match self.node.send_message(peer_id, "/dht/1.0.0", message_data).await {
            Ok(_) => {
    ...
    fn create_response_message(&self, request: &DhtNetworkMessage, result: DhtNetworkResult) -> Result<DhtNetworkMessage> {
        Ok(DhtNetworkMessage {
            ...
            payload: match result {
                ...
                DhtNetworkResult::ValueFound { key, .. } => 
                    DhtNetworkOperation::FindValue { key },
                DhtNetworkResult::PongReceived { .. } => 
                    DhtNetworkOperation::Ping,
                DhtNetworkResult::JoinSuccess { .. } => 
                    DhtNetworkOperation::Join,
                ...
                match event {
                    crate::network::P2PEvent::PeerConnected(peer_id) => {
                        ...
                        // Add to DHT peers
                        {
                            ...
                            peers.insert(peer_id.clone(), DhtPeerInfo {
                                peer_id: peer_id.clone(),
                                dht_key: dht_key.clone(),
                                addresses: vec![],
                                last_seen: Instant::now(),
                    ...
                    crate::network::P2PEvent::Message { topic, source, data } => {
                        if topic == "/dht/1.0.0" {
            ...
            for address in &bootstrap_node.addresses {
                match self.node.connect_peer(&address.to_string()).await {
                    Ok(peer_id) => {
                        ...
                        // Add to DHT peers
                        let dht_key = bootstrap_node.dht_key.clone()
                            .unwrap_or_else(|| Key::new(bootstrap_node.peer_id.as_bytes()));
                        
                        let mut peers = self.dht_peers.write().await;
                        peers.insert(bootstrap_node.peer_id.clone(), DhtPeerInfo {
                    ...
                    Err(e) => {
                        warn!("Failed to connect to bootstrap node {} at {}: {}", 
