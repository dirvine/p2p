...
impl PrometheusExporter {
    ...
    pub async fn export(&self) -> Result<String> {
        ...
        let timestamp = SystemTime::now()
            ...
            .map_err(|e| crate::P2PError::Internal(format!("Failed to get timestamp: {}", e).into()))?
            .as_secs();
    ...
    pub async fn export_metrics(&self) -> Result<HealthMetrics> {
        ...
        for (name, component) in &health.checks {
            match name.as_str() {
                "network" => {
                    if let Some(count) = component.metadata.get("peer_count").and_then(|v| v.as_u64()) {
                        network_peer_count = count;
                    }
                    if let Some(count) = component.metadata.get("active_connections").and_then(|v| v.as_u64()) {
                        active_connections = count;
                    }
                }
                "dht" => {
                    if let Some(size) = component.metadata.get("routing_table_size").and_then(|v| v.as_u64()) {
                        dht_routing_table_size = size;
                    }
                }
                "resources" => {
                    if let Some(ops) = component.metadata.get("dht_ops_per_sec").and_then(|v| v.as_f64()) {
                        dht_ops_per_second = ops;
                    }
                    if let Some(calls) = component.metadata.get("mcp_calls_per_sec").and_then(|v| v.as_f64()) {
                        mcp_calls_per_second = calls;
                    }
                    if let Some(bw) = component.metadata.get("bandwidth_usage").and_then(|v| v.as_u64()) {
                        bandwidth_usage_bps = bw;
                    }
                }
                "storage" => {
                    if let Some(free) = component.metadata.get("free_space").and_then(|v| v.as_u64()) {
...
mod tests {
    ...
    async fn test_prometheus_export_with_components() {
        ...
        health_manager.register_checker("test_component", Box::new(MockChecker)).await;
    ...
    async fn test_prometheus_format_validation() {
        ...
        for line in metrics.lines() {
            ...
            if !line.starts_with('#') {
                ...
                assert!(value.parse::<f64>().is_ok(), "Invalid metric value: {}", value);
            }
        }
    }
    ...
    async fn test_export_with_metadata() {
        ...
        health_manager.register_checker("network", Box::new(MockCheckerWithMetadata)).await;
