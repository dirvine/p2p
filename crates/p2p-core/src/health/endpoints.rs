...
impl HealthEndpoints {
    ...
    pub async fn serve(self, addr: std::net::SocketAddr) -> Result<()> {
        ...
        let listener = TcpListener::bind(addr).await
            .map_err(|e| P2PError::Network(crate::error::NetworkError::BindError(
                format!("Failed to bind health server: {}", e).into()
            )))?;
        
        axum::serve(listener, router)
            .await
            .map_err(|e| P2PError::Network(crate::error::NetworkError::ProtocolError(
...
async fn health_handler(
    ...
    if latency.as_millis() > 100 {
        error!("Health check took {}ms (> 100ms threshold)", latency.as_millis());
    }
...
async fn ready_handler(
    ...
    let health = endpoints.health_manager.get_health().await
        .map_err(|e| HealthError::Internal(e.to_string()))?;
    
    let latency = start.elapsed();
    if latency.as_millis() > 100 {
        error!("Ready check took {}ms (> 100ms threshold)", latency.as_millis());
    }
...
async fn metrics_handler(
    ...
    let metrics = endpoints.prometheus_exporter.export().await
        .map_err(|e| HealthError::Internal(e.to_string()))?;
    
    let latency = start.elapsed();
    if latency.as_millis() > 100 {
        error!("Metrics export took {}ms (> 100ms threshold)", latency.as_millis());
    }
...
async fn debug_handler(
    ...
    let debug_info = endpoints.health_manager.get_debug_info().await
        .map_err(|e| HealthError::Internal(e.to_string()))?;
    
    let latency = start.elapsed();
    if latency.as_millis() > 100 {
        error!("Debug info took {}ms (> 100ms threshold)", latency.as_millis());
    }
...
mod tests {
    use super::*;
    use crate::health::{HealthManager, HealthResponse, DebugInfo};
    use axum::body::Body;
    ...
    async fn test_debug_endpoint() {
        ...
        let response = app
            .oneshot(Request::builder().uri("/debug/vars").body(Body::empty()).unwrap())
            .await
            .unwrap();
