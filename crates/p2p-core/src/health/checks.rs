...
pub struct NetworkHealthChecker {
    get_peer_count: Box<dyn Fn() -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<usize>> + Send>> + Send + Sync>,
    min_peers: usize,
...
pub struct DhtHealthChecker {
    get_routing_table_size: Box<dyn Fn() -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<usize>> + Send>> + Send + Sync>,
    min_nodes: usize,
    timeout_duration: Duration,
}
...
impl ComponentChecker for StorageHealthChecker {
    async fn check(&self) -> Result<HealthStatus> {
        ...
        match timeout(self.timeout_duration, tokio::task::spawn_blocking(move || {
            check_storage_health(&path, min_free)
        })).await {
...
pub struct TransportHealthChecker {
    is_listening: Box<dyn Fn() -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<bool>> + Send>> + Send + Sync>,
    timeout_duration: Duration,
}
...
mod tests {
    ...
    async fn test_composite_health_checker_unhealthy() {
        ...
        let checker = CompositeHealthChecker::new()
            .add_checker("unhealthy", Box::new(AlwaysUnhealthy));
