...
impl CryptoTests {
    ...
    async fn test_basic_crypto_operations(
        ...
        ctx.log_info(&format!(
            "Basic crypto operations completed. Results: {}",
            results.len()
        ));
        Ok(results)
    }
    ...
    async fn test_ed25519_operations(&self, ctx: &TestContext) -> Result<Vec<VerificationResult>> {
        ...
        for (i, message) in test_messages.iter().enumerate() {
            ...
            match keypair.verifying_key().verify(message, &signature) {
                Ok(_) => {
                    ctx.log_info(&format!(
                        "✅ Ed25519 signature verification PASSED for message {}",
                        i + 1
                    ));
                    ...
                    let reconstructed_signature =
                        ed25519_dalek::Signature::try_from(&signature_bytes[..]).map_err(|e| {
                            anyhow::Error::msg(format!("Signature reconstruction failed: {}", e))
                        })?;
                        ...
                        Ok(_) => {
                            results.push(
                                VerificationResult::success(start_time.elapsed())
                                    ...
                                    .with_metadata(
                                        "signature_round_trip".to_string(),
                                        "passed".to_string(),
                                    ),
                            );
                        }
    ...
    async fn test_quantum_resistant_crypto(
        ...
        for (scenario, data) in test_scenarios {
            ...
            if quantum_successful {
                ctx.log_info(&format!(
                    "✅ Quantum-resistant crypto PASSED for {}",
                    scenario
                ));
                results.push(
                    VerificationResult::success(start_time.elapsed())
    ...
    async fn test_hybrid_crypto_system(
        &self,
        ctx: &TestContext,
    ) -> Result<Vec<VerificationResult>> {
        let mut results = Vec::new();
        ...
        for (scenario, scheme) in test_scenarios {
            ...
            if verification_passed {
                ctx.log_info(&format!(
                    "✅ Hybrid crypto verification PASSED for {}",
                    scenario
                ));
                results.push(
                    VerificationResult::success(start_time.elapsed())
    ...
    async fn test_threshold_signatures(
        &mut self,
        ctx: &TestContext,
    ) -> Result<Vec<VerificationResult>> {
        let mut results = Vec::new();
        ...
        for (total_participants, threshold) in threshold_configs {
            ctx.log_info(&format!(
                "[THRESHOLD] Testing {}-of-{} threshold scheme",
                threshold, total_participants
            ));

            let group_result = self
                .test_threshold_group_creation(total_participants, threshold, ctx)
                .await?;
            results.extend(group_result);

            let signing_result = self
                .test_threshold_signing(total_participants, threshold, ctx)
                .await?;
    ...
    async fn test_threshold_group_creation(
        &mut self,
        total: usize,
        threshold: usize,
        ctx: &TestContext,
        ...
        ctx.log_info(&format!(
            "[DKG] Creating {}-of-{} threshold group",
            threshold, total
        ));
        ...
        if dkg_successful {
            ...
            self.threshold_groups
                .insert(group_id.clone(), (total, threshold));

            ctx.log_info(&format!(
                "✅ DKG completed successfully for {}-of-{} group",
                threshold, total
            ));
            results.push(
                VerificationResult::success(start_time.elapsed())
    ...
    async fn test_threshold_signing(
        ...
        for (i, message) in test_messages.iter().enumerate() {
            ...
            ctx.log_info(&format!(
                "[THRESHOLD-SIGN] Message {} with {}-of-{}",
                i + 1,
                threshold,
                total
            ));
            ...
            if signing_successful {
                ctx.log_info(&format!(
                    "✅ Threshold signature PASSED for message {} ({}-of-{})",
                    i + 1,
                    threshold,
                    total
                ));
                ...
                let error = format!(
                    "Threshold signing failed for message {} ({}-of-{})",
                    i + 1,
                    threshold,
                    total
                );
    ...
    async fn test_hierarchical_threshold(
        &mut self,
        ctx: &TestContext,
    ) -> Result<Vec<VerificationResult>> {
        let mut results = Vec::new();
        ...
        for (level, total, threshold) in hierarchy_levels {
            ...
            if hierarchy_successful {
                ctx.log_info(&format!(
                    "✅ Hierarchical threshold PASSED for {} level",
                    level
                ));
                results.push(
                    VerificationResult::success(start_time.elapsed())
                        .with_metadata(
                            "operation".to_string(),
                            "hierarchical_threshold".to_string(),
                        )
