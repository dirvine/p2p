...
use crate::utils::{DataVerifier, TestContext, VerificationResult};
use anyhow::Result;
use saorsa_core::{
    Multiaddr, PeerId,
    dht::Key,
...
impl NetworkTests {
    ...
    async fn test_dht_operations(&self, ctx: &TestContext) -> Result<Vec<VerificationResult>> {
        ...
        let test_data = vec![
            ...
            (
                "test_key_2",
                b"Test data with special chars: !@#$%^&*()".to_vec(),
            ),
        ...
        if let Some(node) = &self.local_node {
            ...
            for (key_str, data) in test_data {
                ...
                match self
                    .store_and_verify_dht_record(node, key.clone(), &data, ctx)
                    .await
                {
        ...
        ctx.log_info(&format!(
            "DHT operations completed. Results: {}",
            results.len()
        ));
        Ok(results)
    }
                ...
                match node.dht_get(key.clone()).await {
                    Ok(Some(retrieved_data)) => {
                        ...
                        if retrieved_data == original_data {
                            ctx.log_info(&format!(
                                "✅ Data verification PASSED for key: {:?}",
                                key
                            ));
                            Ok(VerificationResult::success(duration)
                                .with_metadata(
                                    "operation".to_string(),
                                    "store_and_retrieve".to_string(),
                                )
                                .with_metadata("data_verified".to_string(), "true".to_string())
                                .with_metadata(
                                    "data_size".to_string(),
                            ...
                            let error = format!(
                                "Data corruption detected! Original: {} bytes, Retrieved: {} bytes",
                                original_data.len(),
                                retrieved_data.len()
                            );
                            ctx.log_error(&error);
    ...
    async fn test_connectivity(&self, ctx: &TestContext) -> Result<Vec<VerificationResult>> {
        ...
        ctx.log_info(&format!(
            "Network connectivity tests completed. Results: {}",
            results.len()
        ));
        Ok(results)
    }
    ...
    async fn test_multi_node_network(
        ...
        for i in 0..node_count {
            ...
            match self.create_test_node(port).await {
                ...
                Err(e) => {
                    let error_msg =
                        format!("Failed to create node {} on port {}: {}", i + 1, port, e);
                    ctx.log_error(&error_msg);
                    return Ok(vec![VerificationResult::failure(
                        error_msg,
                        start_time.elapsed(),
                    )]);
                }
        ...
        ctx.log_info(&format!(
            "✅ Successfully created {} nodes in {:?}",
            node_count, creation_time
        ));
        ...
        ctx.log_info(&format!(
            "🎯 {}-node test completed in {:?}",
            node_count, total_time
        ));

        // Add summary result
        results.push(
            VerificationResult::success(total_time)
    ...
    async fn test_cross_node_storage(
        &self,
        nodes: &[Arc<P2PNode>],
        ctx: &TestContext,
    ) -> Result<Vec<VerificationResult>> {
        ...
        ctx.log_info(&format!(
            "🔄 Testing cross-node storage with {} nodes",
            nodes.len()
        ));
        ...
        let test_data = vec![
            ...
            (
                "multi_node_test_2",
                format!("Test with {} nodes", nodes.len()).into_bytes(),
            ),
            (
                "multi_node_binary",
                vec![0xAAu8, 0xBB, 0xCC, 0xDD].repeat(25),
            ),
            (
                "multi_node_large",
                "Large data for multi-node test. ".repeat(50).into_bytes(),
            ),
        ];
            ...
            match storage_node.dht_put(key.clone(), data.clone()).await {
                Ok(_) => {
                        ...
                        match node.dht_get(key.clone()).await {
                            Ok(Some(retrieved_data)) => {
                                if retrieved_data == data {
                                    ...
                                    if i < 5 {
                                        // Log first few successful retrievals
                                        ctx.log_info(&format!(
                                            "✅ Retrieved '{}' from node {} - data verified",
                                            key_str,
                                            i + 1
                                        ));
                                    }
                                    ...
                                    let error = format!(
                                        "Data corruption on node {}: expected {} bytes, got {} bytes",
                                        i + 1,
                                        data.len(),
                                        retrieved_data.len()
                                    );
                    ...
                    if verification_errors.is_empty() {
                        ctx.log_info(&format!(
                            "✅ Cross-node storage test passed: {}/{} successful retrievals",
                        ...
                        results.push(
                            VerificationResult::success(duration)
                                .with_metadata(
                                    "operation".to_string(),
                                    "cross_node_storage".to_string(),
                                )
                                .with_metadata("key".to_string(), key_str.to_string())
                                .with_metadata(
                                    "successful_retrievals".to_string(),
                                    successful_retrievals.to_string(),
                                )
                        ...
                        ctx.log_error(&format!(
                            "❌ Cross-node storage verification failed: {}",
                            error_summary
                        ));
    ...
    async fn test_data_consistency(
        ...
        ctx.log_info(&format!(
            "🔍 Testing data consistency across {} nodes",
            nodes.len()
        ));
        ...
        if storage_successes > 0 {
                ...
                match node.dht_get(key.clone()).await {
                    Ok(Some(retrieved_data)) => {
                        if retrieved_data == consistency_data {
                            ...
                            ctx.log_error(&format!(
                                "❌ Inconsistent data on node {}: expected {} bytes, got {} bytes",
                                i + 1,
                                consistency_data.len(),
                                retrieved_data.len()
                            ));
                        }
                    }
            ...
            if inconsistent_reads == 0 && consistent_reads > 0 {
                ctx.log_info(&format!(
                    "✅ Data consistency verified: {}/{} nodes have consistent data",
                    consistent_reads,
                    nodes.len()
                ));
                results.push(
                ...
                let error = format!(
                    "Data consistency failed: {} consistent, {} inconsistent reads",
    ...
    async fn test_network_resilience(
        ...
        ctx.log_info(&format!(
            "🛡️ Testing network resilience with {} nodes",
            nodes.len()
        ));
        ...
        let storage_node = &nodes[0];
        match storage_node
            .dht_put(resilience_key.clone(), resilience_data.clone())
            .await
        {
            Ok(_) => {
                ...
                match storage_node.dht_get(resilience_key.clone()).await {
                    Ok(Some(retrieved_data)) => {
                        if retrieved_data == resilience_data {
                            ctx.log_info(
                                "✅ Network resilience test passed: data remains accessible",
                            );
                            results.push(
                            ...
                            let error =
                                "Resilience test failed: data corruption detected".to_string();
                            ctx.log_error(&error);
                            results.push(VerificationResult::failure(error, start_time.elapsed()));
                        }
                    }
    ...
    async fn test_basic_connectivity(&self, ctx: &TestContext) -> Result<VerificationResult> {
        ...
        if let Some(node) = &self.local_node {
            ...
            ctx.log_info(&format!(
                "✅ Node connectivity check passed. Peer count: {}",
                peer_count
            ));
            Ok(VerificationResult::success(start_time.elapsed())
    ...
    async fn test_transport(&self, ctx: &TestContext) -> Result<Vec<VerificationResult>> {
        ...
        results.push(
            VerificationResult::success(Duration::from_millis(150))
...
impl SubsystemTest for NetworkTests {
    ...
    async fn test_cross_node(&self, ctx: &TestContext) -> Result<Vec<VerificationResult>> {
        ...
        ctx.log_info(&format!(
            "Cross-node testing completed. Total results: {}",
            results.len()
        ));
        Ok(results)
    }
    ...
    async fn test_stress(&self, ctx: &TestContext) -> Result<Vec<VerificationResult>> {
        ...
        results.push(
            VerificationResult::success(Duration::from_millis(500))
