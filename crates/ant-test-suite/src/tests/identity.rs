...
impl IdentityTests {
    ...
    async fn test_user_profile_operations(&mut self, ctx: &TestContext) -> Result<Vec<VerificationResult>> {
        let mut results = Vec::new();
        
        ctx.log_info("Testing user profile operations");
        
        // Generate test user profiles
        let test_profiles = (0..5).map(|_| self.generator.generate_user_profile()).collect::<Vec<_>>();
        ...
        ctx.log_info(&format!("User profile operations completed. Results: {}", results.len()));
        Ok(results)
    }
    
    /// Test profile serialization and round-trip data integrity
    async fn test_profile_serialization(&self, profile: &UserProfile, ctx: &TestContext) -> Result<VerificationResult> {
        let start_time = std::time::Instant::now();
        ...
        if self.verify_profile_integrity(profile, &deserialized) {
            ctx.log_info(&format!("✅ Profile data integrity verified for user: {}", profile.display_name));
            ...
            let error = "Profile data corruption detected during serialization round-trip".to_string();
            ctx.log_error(&error);
    ...
    async fn test_profile_validation(&self, profile: &UserProfile, ctx: &TestContext) -> Result<VerificationResult> {
        let start_time = std::time::Instant::now();
        ...
        if validation_errors.is_empty() {
            ctx.log_info(&format!("✅ Profile validation passed for user: {}", profile.display_name));
            Ok(VerificationResult::success(start_time.elapsed())
                .with_metadata("operation".to_string(), "profile_validation".to_string())
                .with_metadata("user_id".to_string(), profile.user_id.clone())
                .with_metadata("validation_status".to_string(), "passed".to_string()))
        } else {
            let error = format!("Profile validation failed: {}", validation_errors.join(", "));
            ctx.log_error(&error);
            Ok(VerificationResult::failure(error, start_time.elapsed()))
        }
    }
    ...
    async fn test_user_preferences(&mut self, ctx: &TestContext) -> Result<Vec<VerificationResult>> {
        let mut results = Vec::new();
                    ...
                    match serde_json::from_str::<saorsa_core::identity::manager::UserPreferences>(&serialized) {
                        Ok(deserialized) => {
                            ...
                            results.push(VerificationResult::success(start_time.elapsed())
                                .with_metadata("operation".to_string(), "preferences_serialization".to_string())
    ...
    async fn test_three_word_addresses(&self, ctx: &TestContext) -> Result<Vec<VerificationResult>> {
        let mut results = Vec::new();
...
impl SubsystemTest for IdentityTests {
    ...
    async fn test_basic_functionality(&self, ctx: &TestContext) -> Result<Vec<VerificationResult>> {
        ...
        ctx.log_info(&format!("Basic identity tests completed. Results: {}", results.len()));
        Ok(results)
    }
    ...
    async fn test_data_verification(&self, ctx: &TestContext) -> Result<Vec<VerificationResult>> {
        ...
        ctx.log_info(&format!("Identity data verification completed. Results: {}", preferences_results.len()));
    ...
    async fn test_cross_node(&self, ctx: &TestContext) -> Result<Vec<VerificationResult>> {
        ...
        Ok(vec![VerificationResult::success(Duration::from_millis(200))])
    }
    ...
    async fn test_stress(&self, ctx: &TestContext) -> Result<Vec<VerificationResult>> {
        ...
        Ok(vec![VerificationResult::success(Duration::from_millis(300))])
    }
}
