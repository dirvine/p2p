...
impl AntConnectLauncher {
    ...
    fn new(debug_mode: bool) -> Result<Self> {
        ...
        let flutter_project_path =
            Self::find_flutter_project().context("Could not locate Ant Connect Flutter project")?;
    ...
    fn find_flutter_project() -> Result<PathBuf> {
        ...
        if dev_path.join("pubspec.yaml").exists() {
            info!(
                "Found Flutter project in development location: {:?}",
                dev_path
            );
            return Ok(dev_path);
        }
        ...
        if installed_path.join("pubspec.yaml").exists() {
            info!(
                "Found Flutter project in installed location: {:?}",
                installed_path
            );
    ...
    fn check_flutter_installation(&self) -> Result<()> {
        let output = StdCommand::new("flutter").arg("--version").output();
    ...
    async fn launch_android(&self) -> Result<()> {
        ...
        let devices_output = StdCommand::new("flutter").arg("devices").output()?;
    ...
    async fn launch_embedded_web(&self) -> Result<()> {
        ...
        let server_handle = tokio::spawn(async move { serve_flutter_assets(port).await });
    ...
    async fn execute_flutter_command(&self, mut cmd: StdCommand) -> Result<()> {
        ...
        if status.success() {
            ...
            error!(
                "❌ Flutter process failed with exit code: {:?}",
                status.code()
            );
...
Examples:
  ...
  ant-connect --platform android       # Launch on Android device
    ...
    if matches.get_flag("bootstrap-nodes")
        || matches.get_flag("test-connectivity")
...
async fn perform_comprehensive_connectivity_test(discovery: BootstrapDiscovery) -> Result<()> {
    ...
    if nat_type.contains("Symmetric") || nat_type.contains("Restricted") {
        status.add_warning(format!(
            "NAT type '{}' may require tunnel assistance",
            nat_type
        ));
    }
    ...
    info!("");
    info!(
        "📊 Connectivity Results: {}/{} bootstrap nodes reachable",
...
async fn serve_flutter_assets(port: u16) -> Result<()> {
    let routes = warp::any().and(warp::path::full()).and_then(
        move |path: warp::path::FullPath| async move { serve_embedded_file(path.as_str()).await },
    );
...
async fn serve_embedded_file(path: &str) -> Result<impl warp::Reply + use<>, Infallible> {
    ...
    match None::<&include_dir::File> {
        Some(file) => {
            ...
            let mut response =
                reply::with_header(file.contents(), "content-type", mime_type).into_response();
        ...
        None => {
            ...
            Ok(reply::with_status("File not found", StatusCode::NOT_FOUND).into_response())
        }
    }
}
