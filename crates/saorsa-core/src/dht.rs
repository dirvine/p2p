...
use crate::validation::{
    Validate, ValidationContext, validate_dht_key, validate_dht_value, validate_peer_id,
};
    ...
    fn validate(&self, ctx: &ValidationContext) -> Result<()> {
        ...
        if self.created_at > now {
            return Err(P2PError::validation(
                "Record creation time is in the future",
            ));
        }

        if self.expires_at < self.created_at {
            return Err(P2PError::validation(
                "Record expiration time is before creation time",
            ));
        }
...
impl KBucket {
    ...
    fn add_node(&mut self, node: DHTNode) -> bool {
        ...
        if let Some(pos) = self.nodes.iter().position(|n| n.peer_id == node.peer_id) {
            ...
            let updated_node = if let Some(existing_node) = self.nodes.get_mut(pos) {
                existing_node.touch();
                existing_node.is_connected = node.is_connected;
                existing_node.clone()
            } else {
                return false; // Position invalid (should not happen)
            };
            // Remove and re-add at front
            self.nodes.remove(pos);
            self.nodes.push_front(updated_node);
            return true;
        }
...
impl RoutingTable {
    ...
    pub async fn add_node(&self, node: DHTNode) -> Result<()> {
        ...
        if bucket.add_node(node.clone()) {
            ...
            debug!(
                "Bucket {} full, could not add node {}",
                bucket_index, node.peer_id
            );
        }
...
impl DHT {
        ...
        if let Some(ref mut manager) = self.ipv6_identity_manager {
            ...
            Err(P2PError::Security(
                crate::error::SecurityError::AuthorizationFailed(
                    "IPv6 identity manager not enabled".into(),
                ),
            ))
        }
    }
    ...
    pub async fn add_ipv6_node(
        &mut self,
        peer_id: PeerId,
        addresses: Vec<Multiaddr>,
        ipv6_identity: crate::security::IPv6NodeID,
    ) -> Result<()> {
        if let Some(ref mut manager) = self.ipv6_identity_manager {
            // Validate the node join with IPv6 security checks
            let base_node = DHTNode::new(peer_id.clone(), addresses, &self.local_id);
            let security_event = manager
                .validate_node_join(&base_node, &ipv6_identity)
                .await?;

            match security_event {
                ipv6_identity::IPv6SecurityEvent::NodeJoined {
                    verification_confidence,
                    ..
                } => {
                    // Enhance the node with IPv6 identity
                    let ipv6_node = manager
                        .enhance_dht_node(base_node.clone(), ipv6_identity)
                        .await?;
                    ...
                    info!(
                        "Added IPv6-verified node {} with confidence {:.2}",
                        peer_id, verification_confidence
                    );
                    Ok(())
                }
                ipv6_identity::IPv6SecurityEvent::VerificationFailed { .. } => Err(
                    P2PError::Security(crate::error::SecurityError::AuthenticationFailed),
                ),
                ipv6_identity::IPv6SecurityEvent::DiversityViolation { subnet_type, .. } => Err(
                    P2PError::Security(crate::error::SecurityError::AuthorizationFailed(
                        format!("IP diversity violation: {subnet_type}").into(),
                    )),
                ),
                ipv6_identity::IPv6SecurityEvent::NodeBanned { reason, .. } => Err(
                    P2PError::Security(crate::error::SecurityError::AuthorizationFailed(
                        format!("Node banned: {reason}").into(),
                    )),
                ),
                _ => Err(P2PError::Security(
    ...
    pub async fn put(&self, key: Key, value: Vec<u8>) -> Result<()> {
        ...
        info!(
            "Storing record with key {} on {} nodes",
            key.to_hex(),
            closest_nodes.len()
        );
        ...
        info!(
            "Successfully replicated record {} to {}/{} nodes",
            key.to_hex(),
    ...
    pub async fn handle_query(&self, query: DHTQuery) -> DHTResponse {
        match query {
            ...
            DHTQuery::Ping { requester: _ } => DHTResponse::Pong {
                responder: self.local_id.to_hex(),
            },
        }
    }
        ...
        for node in &secure_nodes {
            ...
            if enable_distance_verification {
                ...
                let consensus = if let Some(ref mut skademlia) = self.skademlia {
                    skademlia
                        .verify_distance_consensus(&node.peer_id, key, witness_nodes)
                        .await?
                } else {
                    continue;
                };
                ...
                if consensus.confidence < min_reputation {
                    debug!(
                        "Skipping node {} due to low distance verification confidence",
                        node.peer_id
                    );
                    continue;
                }
    ...
    pub async fn secure_put(&mut self, key: Key, value: Vec<u8>) -> Result<()> {
        ...
        let secure_nodes = if let Some(ref skademlia) = self.skademlia {
            ...
            skademlia.select_secure_nodes(&candidate_nodes, &key, self.config.replication_factor)
        } else {
        ...
        info!(
            "Storing record with key {} on {} secure nodes",
            key.to_hex(),
            secure_nodes.len()
        );
        ...
        if successful_replications > 0 {
            info!(
                "Successfully replicated to {}/{} secure nodes",
                successful_replications,
                secure_nodes.len()
            );
        }
    ...
    pub async fn update_sibling_list(&mut self, key: Key) -> Result<()> {
        if let Some(ref mut skademlia) = self.skademlia {
            let nodes = self
                .routing_table
                .closest_nodes(&key, skademlia.config.sibling_list_size)
                .await;
    ...
    pub async fn validate_routing_consistency(&self) -> Result<skademlia::ConsistencyReport> {
        if let Some(ref skademlia) = self.skademlia {
            ...
            Err(P2PError::Dht(crate::error::DhtError::RoutingError(
                "S/Kademlia not enabled".to_string().into(),
            )))
        }
    }

    /// Create a distance verification challenge for a peer
    pub fn create_distance_challenge(
        &mut self,
        peer_id: &PeerId,
        key: &Key,
    ) -> Option<skademlia::DistanceChallenge> {
        self.skademlia
            .as_mut()
    ...
    #[allow(dead_code)]
    async fn verify_node_distances(
        &self,
        nodes: &[DHTNode],
        _target_key: &Key,
        min_reputation: f64,
        ...
        for node in nodes {
            ...
            if witness_nodes.len() >= 2 {
                ...
                if consensus_confidence >= min_reputation {
                    ...
                    debug!(
                        "Node {} failed distance verification with confidence {}",
    ...
    async fn select_witness_nodes(&self, target_peer: &PeerId, count: usize) -> Vec<PeerId> {
        ...
        let candidate_nodes = self
            .routing_table
            .closest_nodes(&target_key, count * 2)
            .await;
    ...
    ) -> Option<skademlia::EnhancedDistanceChallenge> {
        self.skademlia.as_mut().map(|skademlia| {
            skademlia.create_adaptive_distance_challenge(peer_id, key, suspected_attack)
        })
    }
    ...
    pub async fn verify_distance_multi_round(
        &mut self,
        challenge: &skademlia::EnhancedDistanceChallenge,
    ) -> Result<bool> {
        if let Some(ref mut skademlia) = self.skademlia {
    ...
    pub async fn ipv6_secure_get(&mut self, key: &Key) -> Result<Option<Record>> {
        ...
        if self.is_node_banned(&self.local_id.to_hex()) {
            return Err(P2PError::Security(
                crate::error::SecurityError::AuthorizationFailed("Local node is banned".into()),
            ));
        }
            ...
            for node in &secure_nodes {
                ...
                if let Some(ref manager) = self.ipv6_identity_manager {
                    if let Some(ipv6_node) = manager.get_verified_node(&node.peer_id) {
                        ...
                        if ipv6_node
                            .needs_identity_refresh(manager.config.identity_refresh_interval)
                        {
                        ...
                        debug!(
                            "Skipping node {} without verified IPv6 identity",
                            node.peer_id
                        );
                        continue;
                    }
    ...
    pub async fn ipv6_secure_put(&mut self, key: Key, value: Vec<u8>) -> Result<()> {
        ...
        info!(
            "Storing record with key {} on {} IPv6-verified secure nodes",
            key.to_hex(),
            secure_nodes.len()
        );
        ...
        info!(
            "Successfully replicated to {}/{} IPv6-verified nodes",
            successful_replications,
            secure_nodes.len()
        );
        Ok(())
    ...
    async fn replicate_record(&self, record: &Record, node: &DHTNode) -> Result<()> {
        ...
        if rand::random::<f64>() < 0.95 {
            ...
            Err(P2PError::Network(
                crate::error::NetworkError::ProtocolError("Replication failed".to_string().into()),
            ))
        }
    }
    ...
    async fn iterative_find_value(&self, key: &Key) -> Option<Record> {
        ...
        let initial_nodes = self
            .routing_table
            .closest_nodes(key, self.config.alpha)
            .await;
            ...
            for query_result in futures::future::join_all(queries).await {
                match query_result {
                    Ok(DHTResponse::Value { record }) => {
                        debug!(
                            "Found value for key {} in iteration {}",
                            key.to_hex(),
                            iterations
                        );
                        return Some(record);
                    ...
                    Ok(DHTResponse::Nodes { nodes }) => {
                        let dht_nodes: Vec<DHTNode> =
                            nodes.into_iter().map(|n| n.to_dht_node()).collect();
        ...
        debug!(
            "Iterative lookup for key {} completed after {} iterations, value not found",
            key.to_hex(),
            iterations
        );
        None
    ...
    async fn republish_records(&self) -> Result<()> {
        ...
        if republished_count > 0 {
            debug!(
                "Republished {} records during maintenance",
                republished_count
            );
        }
    ...
    async fn iterative_find_node(&self, key: &Key) -> Vec<DHTNode> {
        ...
        debug!(
            "Iterative node lookup for key {} completed after {} iterations",
            key.to_hex(),
            iterations
        );
    ...
    pub async fn check_consistency(&self, key: &Key) -> Result<ConsistencyReport> {
        ...
        debug!(
            "Consistency check for key {}: {} nodes queried, {} responded, {} records found, consistent: {}",
            key.to_hex(),
            report.nodes_queried,
            report.nodes_responded,
    ...
    pub async fn repair_record(&self, key: &Key) -> Result<RepairResult> {
        ...
        debug!(
            "Repair for key {} completed: {}/{} repairs successful, final state: {}",
            key.to_hex(),
            repairs_successful,
            repairs_attempted,
            final_state
    ...
    pub async fn create_inbox(&self, inbox_id: &str, owner_peer_id: PeerId) -> Result<InboxInfo> {
        ...
        info!(
            "Successfully created inbox {} with four-word address: {}",
    ...
    pub async fn send_message_to_inbox(&self, inbox_id: &str, message: InboxMessage) -> Result<()> {
        ...
        let inbox_key = Key::from_inbox_id(inbox_id);
        let metadata_record = self.get(&inbox_key).await.ok_or_else(|| {
            P2PError::Dht(crate::error::DhtError::KeyNotFound(
        ...
        let index_key = Key::from_inbox_index(inbox_id);
        let index_record = self.get(&index_key).await.ok_or_else(|| {
            P2PError::Dht(crate::error::DhtError::KeyNotFound(
        ...
        self.put_record_with_infinite_ttl(updated_index_record)
            .await?;
        self.put_record_with_infinite_ttl(updated_metadata_record)
            .await?;

        info!(
            "Successfully sent message {} to inbox {}",
            message.id, inbox_id
        );
        Ok(())
    }
    ...
    async fn put_record_with_infinite_ttl(&self, record: Record) -> Result<()> {
        ...
        for node in &closest_nodes {
            if let Err(e) = self.replicate_record(&record, node).await {
                debug!(
                    "Failed to replicate infinite TTL record to node {}: {}",
                    node.peer_id, e
                );
            }
        }
...
impl LookupState {
    ...
    pub fn add_nodes(&mut self, nodes: Vec<DHTNode>) {
        ...
        self.to_query
            .make_contiguous()
            .sort_by_key(|node| node.key().distance(target).as_bytes().to_vec());
    }
