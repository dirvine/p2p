...
//!
//! Run with: `rustc --edition 2024 src/enhanced_dht_storage_manager_sync.rs && ./enhanced_dht_storage_manager_sync`
    ...
    fn default() -> Self {
        Self {
            default_access_level: DataAccessLevel::UserPrivate {
                user_key_id: "default_user".to_string(),
...
impl Key {
    ...
    pub fn random() -> Self {
        ...
        // Add more entropy with nanosecond precision
        ...
        // Add some additional randomness to the rest of the hash
        let hash_value2 = hasher.finish().wrapping_mul(0x9e3779b97f4a7c15);
        hash[8..16].copy_from_slice(&hash_value2.to_le_bytes());
...
impl EnhancedRecordManager {
    ...
    pub fn store_with_replication(&mut self, record: EnhancedDhtRecord) -> StorageResult<ReplicationResult> {
        ...
        for peer_id in &target_peers {
            ...
            let success = self.simulate_store_to_peer(&record, peer_id);
            if success {
    ...
    fn select_optimal_peers(&self, _key: &Key, count: usize) -> StorageResult<Vec<PeerId>> {
    ...
    fn simulate_store_to_peer(&self, _record: &EnhancedDhtRecord, _peer_id: &PeerId) -> bool {
...
impl EncryptionService {
    ...
    pub fn encrypt(&self, data: Vec<u8>, _access_level: &DataAccessLevel) -> StorageResult<Vec<u8>> {
        // Simulate encryption
    ...
    pub fn decrypt(&self, encrypted_data: Vec<u8>, _credentials: &AccessCredentials) -> StorageResult<Vec<u8>> {
        // Simulate decryption
...
impl LocalStorageManager {
    ...
    pub fn store(&mut self, key: Key, data: Vec<u8>, metadata: StorageMetadata) -> StorageResult<()> {
        // Check cache limits
    ...
    pub fn retrieve(&mut self, key: &Key) -> Option<(Vec<u8>, StorageMetadata)> {
...
impl EnhancedDhtStorageManager {
    ...
    pub fn new(config: StorageManagerConfig) -> StorageResult<Self> {
        ...
        let serialization_service = Arc::new(RwLock::new(
            SerializationService::new(config.serialization_config.clone())
        ));
        
        let local_storage = Arc::new(RwLock::new(
            LocalStorageManager::new(config.cache_config.clone())?
        ));
        
        let event_publisher = Arc::new(EventPublisher::new());
        let statistics = Arc::new(RwLock::new(StorageStatistics::default()));
    ...
    pub fn store<T>(&self, request: StoreRequest<T>) -> StorageResult<StoreResponse>
    where
        T: std::fmt::Debug + Clone + Send + Sync + 'static,
    {
        ...
        let result = self.store_internal(request, operation_id.clone(), start_time);
    ...
    fn store_internal<T>(&self, request: StoreRequest<T>, operation_id: OperationId, start_time: Instant) -> StorageResult<StoreResponse>
    where
        T: std::fmt::Debug + Clone + Send + Sync + 'static,
    {
        ...
        let serialized_data = {
            let mut serializer = self.serialization_service.write().unwrap();
        ...
        let replication_result = {
            let mut manager = self.replication_manager.write().unwrap();
            manager.store_with_replication(dht_record)?
        };
    ...
    pub fn retrieve<T>(&self, request: RetrieveRequest) -> StorageResult<RetrieveResponse<T>>
    where
        T: std::fmt::Debug + Clone + Send + Sync + 'static,
    {
        let start_time = Instant::now();
        ...
        let result = self.retrieve_internal(request, operation_id.clone(), start_time);
    ...
    fn retrieve_internal<T>(&self, request: RetrieveRequest, operation_id: OperationId, start_time: Instant) -> StorageResult<RetrieveResponse<T>>
    where
        T: std::fmt::Debug + Clone + Send + Sync + 'static,
    {
        ...
        if request.prefer_local_cache {
            let mut local_storage = self.local_storage.write().unwrap();
            if let Some((cached_data, metadata)) = local_storage.retrieve(&request.key) {
        ...
        if data_and_metadata.is_none() {
            ...
            data_and_metadata = self.retrieve_from_dht(&request.key)?;
        }
        ...
        match data_and_metadata {
            Some((encrypted_data, metadata)) => {
                // Decrypt the data
                let _decrypted_data = self.encryption_service
                    .decrypt(encrypted_data, &request.access_credentials)?;
                
                // For demo purposes, return None as we can't deserialize without real data
                let performance_metrics = OperationMetrics {
                    duration: total_duration,
    ...
    fn retrieve_from_dht(&self, _key: &Key) -> StorageResult<Option<(Vec<u8>, StorageMetadata)>> {
    ...
    fn serialize_data<T>(&self, serializer: &mut SerializationService, data: &T, _format: Option<SerializationFormat>) -> StorageResult<SerResult>
    where
        T: std::fmt::Debug,
    {
        // Convert debug representation to bytes for serialization service
        let debug_data = format!("{:?}", data);
        let bytes = debug_data.into_bytes();
        
        let result = serializer.serialize_auto(&bytes, Some(ContentType::Internal))
    ...
    pub fn get_statistics(&self) -> StorageStatistics {
        ...
        StorageStatistics {
            ...
            total_bytes_retrieved: stats.total_bytes_retrieved,
            average_operation_time: stats.average_operation_time,
            cache_hit_rate: stats.cache_hit_rate,
            replication_success_rate: stats.replication_success_rate,
        }
    }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match storage_manager.store(store_request) {
        Ok(response) => {
            ...
            match storage_manager.retrieve::<TestData>(retrieve_request) {
                Ok(retrieve_response) => {
                    ...
                    if let Some(data) = retrieve_response.data {
                        ...
                        println!("   Retrieved data: [Demo - data not fully deserialized]");
                    }
                }
                Err(e) => {
    ...
    // Test statistics
    println!("\n📊 Storage Statistics:");
    let stats = storage_manager.get_statistics();
    ...
    println!("\n💾 Cache Statistics:");
    let cache_stats = storage_manager.get_cache_stats();
    ...
    if let Some(status) = storage_manager.get_replication_status(&key) {
...
mod tests {
    ...
    fn test_storage_manager_creation() {
        let config = StorageManagerConfig::default();
        let manager = EnhancedDhtStorageManager::new(config);
    ...
    fn test_cache_operations() {
        ...
        let mut cache = LocalStorageManager::new(cache_config).unwrap();
        ...
        // Test store
        cache.store(key.clone(), data.clone(), metadata).unwrap();
    ...
    fn test_encryption_service() {
        ...
        let encrypted = service.encrypt(data.clone(), &access_level).unwrap();
        ...
        let decrypted = service.decrypt(encrypted, &credentials).unwrap();
