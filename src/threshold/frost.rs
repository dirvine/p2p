...
impl FrostCoordinator {
    ...
    pub fn register_group(
        ...
        self.groups.insert(group_id, GroupInfo {
            group_public_key,
            threshold,
            participants,
        });
    ...
    pub fn initiate_signing(
        ...
        let group_info = self.groups.get(group_id)
            .ok_or_else(|| ThresholdError::GroupOperationFailed(
    ...
    pub fn complete_signing(&mut self, session_id: &[u8; 32]) -> Result<FrostSignature> {
        let session = self.sessions.get_mut(session_id)
            .ok_or_else(|| ThresholdError::AggregationFailed(
                "Session not found".to_string()
            ))?;
    ...
    pub fn cleanup_old_sessions(&mut self, max_age: std::time::Duration) {
        ...
        self.sessions.retain(|_, session| {
            ...
            !matches!(session.state, SessionState::Completed | SessionState::Failed(_))
        });
    }
}
...
pub async fn generate_key_shares(
    ...
    // Use frost crate for actual key generation
    let (shares, pubkey_package) = frost::keys::generate_with_dealer(
        participants,
        threshold,
        frost::keys::IdentifierList::Default,
        &mut OsRng,
    ).map_err(|e| ThresholdError::DkgFailed(e.to_string()))?;
    
    // Convert to our types
    let mut participant_shares = HashMap::new();
    
    for (i, (identifier, signing_share)) in shares.into_iter().enumerate() {
        let participant_id = ParticipantId(i as u16);
        let verifying_share = pubkey_package.verifying_shares().get(&identifier)
            .ok_or_else(|| ThresholdError::DkgFailed(
                "Missing verifying share".to_string()
            ))?;
    ...
    let group_public_key = FrostGroupPublicKey(
...
mod tests {
    ...
    fn test_frost_session_lifecycle() {
        ...
        session.add_share(
            ParticipantId(1),
            SigningShare { share: vec![5; 32] },
        ).unwrap();
        
        session.add_share(
            ParticipantId(2),
            SigningShare { share: vec![6; 32] },
        ).unwrap();
