...
pub async fn run_ceremony(
    ...
    // For this example, we'll take the first share as the local share
    // In a real implementation, shares would be distributed securely
    let (identifier, signing_share) = shares.into_iter().next()
        .ok_or_else(|| ThresholdError::DkgFailed("No shares generated".to_string()))?;
    
    let local_share = FrostKeyShare {
        participant_id: ParticipantId(0),
        share: signing_share.serialize().to_vec(),
    };
    
    let group_key = FrostGroupPublicKey(
...
mod tests {
    ...
    fn test_dkg_coordinator() {
        ...
        // Process commitments
        coordinator.process_commitment(
            &ceremony_id,
            ParticipantId(1),
            vec![1; 32],
        ).unwrap();
