...
use saorsa_lib::passkey_auth::{MockAuthenticator, PasskeyAuthManager, PlatformAuthenticator};
...
fn create_test_manager(should_succeed: bool) -> (PasskeyAuthManager, TempDir) {
    ...
    manager.authenticator = PlatformAuthenticator::Mock(MockAuthenticator::new(should_succeed));
    (manager, temp_dir)
}
...
mod passkey_tests {
    ...
    async fn test_keychain_storage() {
        ...
        let credential = manager
            .create_passkey("keychain_test", "test.word.address")
            .await
            .unwrap();
        ...
        // Test deletion
        manager
            .delete_passkey(&credential.credential_id)
            .await
...
mod platform_specific_tests {
    ...
    async fn test_platform_detection() {
        ...
        assert!(
            manager.is_ok(),
            "Should create manager on any supported platform"
        );
...
mod cross_platform_tests {
    ...
    async fn test_platform_compatibility() {
        ...
        match os {
            "macos" | "windows" | "linux" => {
                assert!(
                    manager.is_ok(),
                    "Should create manager on supported platform: {}",
                    os
                );
            }
            _ => {
                ...
                assert!(
                    manager.is_ok(),
                    "Should fallback to mock on unsupported platform: {}",
                    os
                );
            }
        }
    ...
    async fn test_credential_id_uniqueness() {
        ...
        for i in 0..10 {
            let cred = manager
                .create_passkey(&format!("user{}", i), &format!("user{}.test.address", i))
                .await
                .unwrap();

            assert!(
                credential_ids.insert(cred.credential_id.clone()),
                "Credential ID should be unique: {}",
                cred.credential_id
            );
        }
...
mod security_tests {
    ...
    async fn test_signature_verification() {
        ...
        let credential = manager
            .create_passkey("security_test", "test.address")
            .await
            .unwrap();
    ...
    async fn test_key_isolation() {
        ...
        let cred1 = manager
            .create_passkey("user1", "user1.address")
            .await
            .unwrap();
        let cred2 = manager
            .create_passkey("user2", "user2.address")
            .await
            .unwrap();

        // Should not be able to authenticate cred1 with cred2's ID
        let wrong_auth = manager
            .authenticate_with_passkey(&cred2.credential_id)
            .await;
