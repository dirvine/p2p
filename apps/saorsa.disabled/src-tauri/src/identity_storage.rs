...
use saorsa_core::identity::{EncryptedUserProfile, UserIdentity};
use saorsa_core::error::CryptoError;
...
impl IdentityStorage {
    ...
    pub fn new(app_handle: AppHandle, config: IdentityStorageConfig) -> Result<Self> {
        ...
        std::fs::create_dir_all(&app_dir).map_err(|e| P2PError::Io(e))?;
    ...
    async fn encrypt_data(&self, data: &[u8], nonce: &[u8; 12]) -> Result<Vec<u8>> {
        ...
        let key = key_guard
            .as_ref()
            .ok_or_else(|| P2PError::Crypto(CryptoError::EncryptionFailed("Encryption not initialized".into())))?;

        let cipher = Aes256Gcm::new_from_slice(key)
            .map_err(|e| P2PError::Crypto(CryptoError::EncryptionFailed(format!("Failed to create cipher: {}", e).into())))?;

        let nonce = Nonce::from_slice(nonce);
        cipher
            .encrypt(nonce, data)
            .map_err(|e| P2PError::Crypto(CryptoError::EncryptionFailed(format!("Encryption failed: {}", e).into())))
    }
    ...
    async fn decrypt_data(&self, encrypted_data: &[u8], nonce: &[u8; 12]) -> Result<Vec<u8>> {
        ...
        cipher
            ...
            .map_err(|e| P2PError::Crypto(CryptoError::DecryptionFailed(format!("Decryption failed: {}", e).into())))
    }
    ...
    pub async fn save_identity(
        ...
        // Serialize data
        let identity_bytes = bincode::serialize(identity).map_err(|e| {
            P2PError::Serialization(format!("Failed to serialize identity: {}", e).into())
        })?;
        let signing_key_bytes = signing_key.to_bytes();
        let profile_bytes = if let Some(p) = profile {
            Some(bincode::serialize(p).map_err(|e| {
                P2PError::Serialization(format!("Failed to serialize profile: {}", e).into())
            })?)
        } else {
            None
        };
        ...
        let passkey_bytes = if !credentials.is_empty() {
            Some(bincode::serialize(&*credentials).map_err(|e| {
                P2PError::Serialization(format!("Failed to serialize passkey credentials: {}", e).into())
            })?)
        } else {
            None
        };
        ...
        let encrypted_signing_key = self
            .encrypt_data(&signing_key_bytes, &signing_key_nonce)
            .await?;
        let encrypted_profile = if let Some(pb) = profile_bytes {
            Some(
                self.encrypt_data(&pb, profile_nonce.as_ref().unwrap())
                    .await?,
            )
        } else {
            None
        ...
        let encrypted_passkey_credentials = if let Some(pb) = passkey_bytes {
            Some(
                self.encrypt_data(&pb, passkey_nonce.as_ref().unwrap())
                    .await?,
            )
        } else {
            None
        ...
        let stored_data = StoredIdentityData {
            ...
            encrypted_passkey_credentials: encrypted_passkey_credentials
        ...
        // Write to file
        let json =
            serde_json::to_string_pretty(&stored_data).map_err(|e| P2PError::from(e))?;
    ...
    pub async fn load_identity(
        ...
        // Read storage file
        let json = std::fs::read_to_string(&self.storage_path).map_err(|e| {
            P2PError::Io(std::io::Error::new(
                std::io::ErrorKind::Other,
        ...
        let stored_data: StoredIdentityData = serde_json::from_str(&json).map_err(|e| {
            P2PError::Serialization(format!("Failed to parse storage data: {}", e).into())
        })?;

        // Decode salt and derive key
        let salt = general_purpose::STANDARD
            .decode(&stored_data.salt)
            .map_err(|e| P2PError::Serialization(format!("Failed to decode salt: {}", e).into()))?;
        let (_, key) = self.derive_key(password, Some(salt))?;
        *self.encryption_key.write().await = Some(key);

        // Decode base64 data
        let encrypted_identity = general_purpose::STANDARD
            .decode(&stored_data.encrypted_identity)
            .map_err(|e| P2PError::Serialization(format!("Failed to decode identity: {}", e).into()))?;
        let encrypted_signing_key = general_purpose::STANDARD
            .decode(&stored_data.encrypted_signing_key)
            .map_err(|e| {
                P2PError::Serialization(format!("Failed to decode signing key: {}", e).into())
            })?;

        // Decode nonces
        let identity_nonce_bytes = general_purpose::STANDARD
            .decode(&stored_data.identity_nonce)
            .map_err(|e| {
                P2PError::Serialization(format!("Failed to decode identity nonce: {}", e).into())
            })?;
        let signing_key_nonce_bytes = general_purpose::STANDARD
            .decode(&stored_data.signing_key_nonce)
            .map_err(|e| {
                P2PError::Serialization(format!("Failed to decode signing key nonce: {}", e).into())
            })?;
        ...
        let identity_bytes = self
            .decrypt_data(&encrypted_identity, &identity_nonce)
            .await?;
        let signing_key_bytes = self
            .decrypt_data(&encrypted_signing_key, &signing_key_nonce)
            .await?;

        // Deserialize
        let identity: UserIdentity = bincode::deserialize(&identity_bytes).map_err(|e| {
            P2PError::Serialization(format!("Failed to deserialize identity: {}", e).into())
        })?;
        // Convert bytes to array and create signing key
        let mut key_array = [0u8; 32];
        if signing_key_bytes.len() != 32 {
            return Err(P2PError::Crypto(CryptoError::InvalidKeyLength {
                expected: 32,
        ...
        let profile = if let Some(encrypted_profile_str) = stored_data.encrypted_profile {
            let encrypted_profile = general_purpose::STANDARD
                .decode(&encrypted_profile_str)
                .map_err(|e| {
                    P2PError::Serialization(format!("Failed to decode profile: {}", e).into())
                })?;

            let profile_nonce_str = stored_data
                .profile_nonce
                .ok_or_else(|| P2PError::Io(std::io::Error::new(std::io::ErrorKind::NotFound, "Missing profile nonce")))?;
            let profile_nonce_bytes = general_purpose::STANDARD
                .decode(&profile_nonce_str)
                .map_err(|e| {
                    P2PError::Serialization(format!("Failed to decode profile nonce: {}", e).into())
                })?;
            ...
            let profile_bytes = self
                .decrypt_data(&encrypted_profile, &profile_nonce)
                .await?;
            let profile: EncryptedUserProfile =
                bincode::deserialize(&profile_bytes).map_err(|e| {
                    P2PError::Serialization(format!("Failed to deserialize profile: {}", e).into())
                })?;
        ...
        if let Some(encrypted_passkey_str) = stored_data.encrypted_passkey_credentials {
            let encrypted_passkey = general_purpose::STANDARD
                .decode(&encrypted_passkey_str)
                .map_err(|e| {
                    P2PError::Serialization(format!("Failed to decode passkey credentials: {}", e).into())
                })?;

            let passkey_nonce_str = stored_data
                .passkey_nonce
                .ok_or_else(|| P2PError::Io(std::io::Error::new(std::io::ErrorKind::NotFound, "Missing passkey nonce")))?;
            let passkey_nonce_bytes = general_purpose::STANDARD
                .decode(&passkey_nonce_str)
                .map_err(|e| {
                    P2PError::Serialization(format!("Failed to decode passkey nonce: {}", e).into())
                })?;

            let mut passkey_nonce = [0u8; 12];
            passkey_nonce.copy_from_slice(&passkey_nonce_bytes);

            let passkey_bytes = self
                .decrypt_data(&encrypted_passkey, &passkey_nonce)
                .await?;
            let credentials: Vec<StoredPasskeyCredential> = bincode::deserialize(&passkey_bytes)
                .map_err(|e| {
                    P2PError::Serialization(format!("Failed to deserialize passkey credentials: {}", e).into())
                })?;
    ...
    pub async fn delete_identity(&self) -> Result<()> {
        if self.storage_path.exists() {
            std::fs::remove_file(&self.storage_path).map_err(|e| {
                P2PError::Io(std::io::Error::new(
                    std::io::ErrorKind::Other,
                    format!("Failed to delete identity file: {}", e),
                ))
            })?;
    ...
    pub async fn add_passkey_credential(
        ...
        if self.identity_exists() {
            ...
            if let Ok(Some((identity, signing_key, profile))) = self.load_identity(password).await {
                self.save_identity(&identity, &signing_key, profile.as_ref(), password)
                    .await?;
            }
        }
    ...
    pub async fn remove_passkey_credential(
        &self,
        credential_id: &str,
        password: &str,
    ) -> Result<bool> {
        ...
        if removed {
            ...
            if self.identity_exists() {
                if let Ok(Some((identity, signing_key, profile))) =
                    self.load_identity(password).await
                {
                    self.save_identity(&identity, &signing_key, profile.as_ref(), password)
                        .await?;
                }
