...
use {
    tokio::sync::oneshot,
    windows::{Foundation::*, Security::Credentials::UI::*, core::*},
};
...
impl WindowsHelloAuth {
    ...
    pub async fn verify_user(&self, message: &str) -> Result<()> {
        #[cfg(target_os = "windows")]
        {
            if !self.available {
                return Err(anyhow::anyhow!(
                    "Windows Hello not available on this device"
                ));
            }
            ...
            tokio::task::spawn_blocking(move || {
                let result = match UserConsentVerifier::RequestVerificationAsync(&h_message) {
                    Ok(operation) => match operation.get() {
                        Ok(consent_result) => match consent_result {
                            ...
                            UserConsentVerificationResult::NotConfiguredForUser => Err(
                                anyhow::anyhow!("Windows Hello not configured for this user"),
                            ),
                    ...
                    Err(e) => {
                        ...
                        Err(anyhow::anyhow!(
                            "Failed to request Windows Hello verification"
                        ))
                    }
                };
        ...
        {
            Err(anyhow::anyhow!(
                "Windows Hello not available on this platform"
            ))
        }
    }
}
