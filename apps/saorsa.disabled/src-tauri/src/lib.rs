...
use saorsa_core::{
    Multiaddr, PeerId, Result as P2PResult,
    dht::{DHT, DHTConfig, Key, Record},
    identity::manager::{IdentityManager, IdentityManagerConfig},
    identity::{
        DefaultPermissions, DiscoverabilitySettings, EncryptedUserProfile, PrivacySettings,
        ProfilePermissions, UserIdentity, UserPreferences, UserProfile, VerificationLevel,
    },
    network::{DHTConfig as NetworkDHTConfig, NodeConfig, P2PNode, SecurityConfig, TrustLevel},
...
impl Default for AppState {
    fn default() -> Self {
        Self {
            ...
            contact_categories: RwLock::new(vec![
                "Friends".to_string(),
                "Family".to_string(),
                "Work".to_string(),
            ]),
...
async fn init_network(
    ...
    let config = NodeConfig {
        ...
        max_connections: if cfg!(target_os = "ios") || cfg!(target_os = "android") {
            50
        } else {
            100
        },
        max_incoming_connections: if cfg!(target_os = "ios") || cfg!(target_os = "android") {
            25
        } else {
            50
        },
    ...
    match P2PNode::new(config).await {
        Ok(mut network) => {
            ...
            *net_guard = Some(network_arc.clone());
            ...
            info!(
                "Network startup complete - identity address updates will happen on user interaction"
            );
...
async fn connect_peer(
    ...
    if let Some(network) = net_guard.as_ref() {
        ...
        let peer_info = if address.contains('.') && address.split('.').count() == 3 {
            ...
            // Connect to peer
            network
                .connect_peer(&multiaddr.to_string())
                .await
...
async fn send_message(
    ...
    let net_guard = state.network.read().await;
    let network = net_guard.as_ref().ok_or("Network not initialized")?;
    ...
    let our_user_id = if let Some(identity_manager) = identity_guard.as_ref() {
        None.map(|i: &UserIdentity| i.user_id.clone())
    ...
    if contact_id == "system" {
        ...
        if let Some(contact) = contacts.get(&contact_id) {
            ...
            if let Some(dht) = network.dht() {
                ...
                dht_guard
                    .put(recipient_key, message_value)
                    .await
            ...
            if contact.is_online {
                ...
                network
                    ...
                    .await
                    .ok(); // Don't fail if direct send fails, we have DHT backup
            }
        } else {
...
async fn send_call_offer(
    ...
    info!(
        "Sending call offer to user: {} for channel: {}",
        user_id, channel_id
    );
...
async fn send_call_answer(
    ...
    info!(
        "Sending call answer to user: {} for channel: {}",
        user_id, channel_id
    );
...
async fn send_ice_candidate(
    ...
    if let Some(contact) = contacts.get(&user_id) {
        if let Some(dht) = network.dht() {
            ...
            dht_guard
                .put(signal_record.key, signal_record.value)
                .await
...
async fn end_call(
    ...
    info!(
        "Ending call with user: {} for channel: {} - reason: {}",
...
/// Create a DHT inbox
#[tauri::command]
async fn create_inbox(state: State<'_, AppState>, inbox_name: String) -> Result<String, String> {
    use rand::Rng;
    ...
    let identity_guard = state.identity_manager.read().await;
    let identity_manager = identity_guard
        .as_ref()
    ...
    return Err(
        "Create inbox not implemented - identity management integration needed".to_string(),
    );
        ...
        match storage.load_identity("temp_password_change_me").await {
            Ok(Some((identity, _keypair, profile))) => {
                ...
                response.insert(
                    ...
                    serde_json::Value::String(identity.display_name_hint.clone()),
                );
                response.insert(
                    "three_word_address".to_string(),
                    serde_json::Value::String(identity.three_word_address.clone()),
                );
...
async fn get_user_profile(state: State<'_, AppState>) -> Result<Option<serde_json::Value>, String> {
    ...
    let identity_manager = identity_manager_guard
        .as_ref()
    ...
    match profile_result {
        Ok(Some(profile)) => {
            ...
            discovery.insert(
                "discoverable_by_name".to_string(),
                serde_json::Value::Bool(profile.preferences.discovery.discoverable_by_name),
            );
            discovery.insert(
                "discoverable_by_friends".to_string(),
                serde_json::Value::Bool(profile.preferences.discovery.discoverable_by_friends),
            );
            discovery.insert(
                "allow_contact_requests".to_string(),
                serde_json::Value::Bool(profile.preferences.discovery.allow_contact_requests),
            );
            discovery.insert(
                "require_mutual_friends".to_string(),
                serde_json::Value::Bool(profile.preferences.discovery.require_mutual_friends),
            );
            discovery.insert(
                "listed_in_directory".to_string(),
                serde_json::Value::Bool(profile.preferences.discovery.listed_in_directory),
            );
            preferences.insert(
                "discovery".to_string(),
                serde_json::Value::Object(discovery),
            );

            // Default permissions
            let mut permissions = serde_json::Map::new();
            permissions.insert(
                "can_see_display_name".to_string(),
                serde_json::Value::Bool(
                    profile.preferences.default_permissions.can_see_display_name,
                ),
            );
            permissions.insert(
                "can_see_avatar".to_string(),
                serde_json::Value::Bool(profile.preferences.default_permissions.can_see_avatar),
            );
            permissions.insert(
                "can_see_status".to_string(),
                serde_json::Value::Bool(profile.preferences.default_permissions.can_see_status),
            );
            permissions.insert(
                "can_see_contact_info".to_string(),
                serde_json::Value::Bool(
                    profile.preferences.default_permissions.can_see_contact_info,
                ),
            );
            permissions.insert(
                "can_see_last_seen".to_string(),
                serde_json::Value::Bool(profile.preferences.default_permissions.can_see_last_seen),
            );
            permissions.insert(
                "can_see_custom_fields".to_string(),
                serde_json::Value::Bool(
                    profile
                        .preferences
                        .default_permissions
                        .can_see_custom_fields,
                ),
            );
            preferences.insert(
            ...
            privacy.insert(
                "require_proof_of_humanity".to_string(),
                serde_json::Value::Bool(profile.preferences.privacy.require_proof_of_humanity),
            );
            privacy.insert(
                "max_contact_request_age".to_string(),
                serde_json::Value::Number(serde_json::Number::from(
                    profile
                        .preferences
                        .privacy
                        .max_contact_request_age
                        .as_secs(),
                )),
            );
            privacy.insert(
                "enable_forward_secrecy".to_string(),
                serde_json::Value::Bool(profile.preferences.privacy.enable_forward_secrecy),
            );
            privacy.insert(
                "auto_rotate_keys".to_string(),
                serde_json::Value::Bool(profile.preferences.privacy.auto_rotate_keys),
            );
            privacy.insert(
                "key_rotation_interval".to_string(),
                serde_json::Value::Number(serde_json::Number::from(
                    profile.preferences.privacy.key_rotation_interval.as_secs(),
                )),
            );
            ...
            // Custom fields
            let custom_fields: serde_json::Map<String, serde_json::Value> =
                profile.custom_fields.into_iter().collect();
            response.insert(
...
async fn create_user_identity(
    ...
    info!(
        "Creating unique network identity: {} -> {}",
        display_name, three_word_address
    );
    ...
    if let Some(network) = network {
        match check_name_availability(network, &display_name).await {
            ...
            Err(e) => {
                warn!(
                    "Failed to check name availability: {} - proceeding anyway",
                    e
                );
            }
        }
        ...
        Ok(identity) => {
            ...
            let signed_packet = if let Some(network) = network {
                ...
                .await
                {
                    ...
                    Err(e) => {
                        warn!(
                            "Failed to create signed identity packet: {} - identity created locally",
                            e
                        );
                        None
                    }
            ...
            if let (Some(network), Some(packet)) = (network, &signed_packet) {
                match register_identity_by_name(network, packet).await {
                    ...
                    Err(e) => {
                        warn!(
                            "Failed to register identity: {} - identity created locally",
                            e
                        );
                    }
                }
                ...
                match register_three_word_address(network, &three_word_address, &identity.user_id)
                    .await
                {
                    Ok(_) => {
                    ...
                    Err(e) => {
                        warn!(
                            "Failed to register three-word address: {} (continuing anyway)",
                            e
                        );
                    }
                }
            ...
            response.insert(
                "user_id".to_string(),
                serde_json::Value::String(identity.user_id),
            );
            response.insert(
                "display_name_hint".to_string(),
                serde_json::Value::String(identity.display_name_hint),
            );
            response.insert(
                "three_word_address".to_string(),
                serde_json::Value::String(identity.three_word_address),
            );
            response.insert(
                "verification_level".to_string(),
                serde_json::Value::String(format!("{:?}", identity.verification_level)),
            );
            response.insert(
                "public_key".to_string(),
                serde_json::Value::String(base64::encode(&identity.public_key)),
            );
            response.insert(
                "network_published".to_string(),
...
async fn import_user_identity(
    ...
    let encoded_data = export_wrapper
        .get("data")
        .and_then(|v| v.as_str())
    ...
    match identity_manager
        .import_identity(&export_data, "default_password")
        .await
    {
        Ok(identity) => {
            ...
            Ok(format!(
                "Identity imported successfully. User ID: {}",
                identity.user_id
            ))
        }
        Err(e) => {
...
async fn bind_ipv6_identity(state: State<'_, AppState>) -> Result<String, String> {
    ...
    // Get current identity
    let current_identity: UserIdentity = None.ok_or("No current identity set")?;

    // Get the node's IPv6 address
    let local_addrs = network.listen_addrs().await;
    let ipv6_addr = local_addrs
        .iter()
        .find(|addr| {
    ...
    if let Some(dht) = network.dht() {
        ...
        dht_guard
            .put(binding_record.key, binding_record.value)
            .await
            .map_err(|e| format!("Failed to store IPv6 binding: {}", e))?;

        info!(
            "IPv6 identity binding created for {} -> {}",
            current_identity.user_id, ipv6_addr
        );

        Ok(format!(
            "IPv6 identity bound successfully\nAddress: {}",
            ipv6_addr
        ))
    } else {
        Err("DHT not available".to_string())
        ...
        if let Some(record) = dht_guard.get(&name_search_key).await {
            ...
            if let Ok(packet) = serde_json::from_slice::<NameSignedPacket>(&record.value) {
                ...
                if packet.name.to_lowercase().contains(&query.to_lowercase()) {
                    if let Ok(identity) =
                        serde_json::from_value::<UserIdentity>(packet.identity_data.clone())
                    {
                        results.push(serde_json::json!({
                            "user_id": identity.user_id,
...
async fn send_contact_request(
    ...
    let our_identity: UserIdentity = None.ok_or("No current identity set")?;
    ...
    if let Some(dht) = network.dht() {
        ...
        let request_key =
            Key::new(format!("contact_request_{}_{}", user_id, request_id).as_bytes());
        ...
        dht_guard
            .put(request_record.key, request_record.value)
            .await
        ...
        contact_requests.sent.push(
            serde_json::from_value(contact_request.clone()).unwrap_or_else(|_| ContactRequest {
...
async fn accept_contact_request(
    ...
    let mut contact_requests = state.contact_requests.write().await;
    let request_index = contact_requests
        .received
        .iter()
    ...
    if let Some(network) = net_guard.as_ref() {
        if let Some(dht) = network.dht() {
            ...
            let acceptance_key = Key::new(
                format!("contact_acceptance_{}_{}", request.from_user_id, request_id).as_bytes(),
            );
            ...
            dht_guard
                .put(acceptance_record.key, acceptance_record.value)
                .await
                .ok();
        }
    }
...
async fn reject_contact_request(
    ...
    let mut contact_requests = state.contact_requests.write().await;
    if let Some(request) = contact_requests
        .received
        .iter_mut()
        .find(|r| r.request_id == request_id)
    {
        ...
        if let Some(network) = net_guard.as_ref() {
            if let Some(dht) = network.dht() {
                ...
                let rejection_key = Key::new(
                    format!("contact_rejection_{}_{}", request.from_user_id, request_id).as_bytes(),
                );
                ...
                dht_guard
                    .put(rejection_record.key, rejection_record.value)
                    .await
                    .ok();
            }
        }
        ...
        .find(|r| r.request_id == request_id)
    {
        ...
        if let Some(network) = net_guard.as_ref() {
            if let Some(dht) = network.dht() {
                ...
                let cancel_key = Key::new(
                    format!("contact_cancel_{}_{}", request.to_user_id, request_id).as_bytes(),
                );
                ...
                dht_guard
                    .put(cancel_record.key, cancel_record.value)
                    .await
                    .ok();
            }
        }
...
/// Delete a contact
#[tauri::command]
async fn delete_contact(state: State<'_, AppState>, contact_id: String) -> Result<(), String> {
...
/// Block a user
#[tauri::command]
async fn block_user(state: State<'_, AppState>, user_id: String) -> Result<(), String> {
...
/// Unblock a user
#[tauri::command]
async fn unblock_user(state: State<'_, AppState>, user_id: String) -> Result<(), String> {
...
#[tauri::command]
async fn add_contact_category(state: State<'_, AppState>, category: String) -> Result<(), String> {
...
fn create_frontend_protocol_handler<R: tauri::Runtime>() -> impl Fn(
    tauri::UriSchemeContext<'_, R>,
...
async fn register_identity_by_name(
    ...
    match network.dht_put(dht_key, packet_data).await {
        Ok(_) => {
            info!(
                "Signed identity registered for name: {}",
                packet.display_name
            );
            Ok(())
        }
        ...
        Ok(user_id) => {
            ...
            match lookup_user_identity(network, &user_id).await {
                Ok(profile) => {
                    ...
                    response.insert(
                        "three_word_address".to_string(),
                        serde_json::Value::String(three_word_address),
                    );
                    response.insert(
                        "status_message".to_string(),
                        profile
...
async fn lookup_user_by_id(
    ...
    match lookup_user_identity(network, &user_id).await {
        Ok(profile) => {
            ...
            response.insert(
                "user_id".to_string(),
                serde_json::Value::String(profile.user_id),
            );
            response.insert(
                "display_name".to_string(),
                serde_json::Value::String(profile.display_name),
            ...
            response.insert(
                "public_key".to_string(),
                serde_json::Value::String(base64::encode(&profile.public_key)),
            );
            response.insert(
                "created_at".to_string(),
                serde_json::Value::String(
...
async fn search_network_users(
    ...
    if results.is_empty() {
        let variations = vec![
            ...
            query
                .chars()
                .take(query.len().saturating_sub(1))
                .collect::<String>(), // remove last char
        ];
                ...
                match lookup_user_by_name(network, &variation).await {
                    Ok(Some(packet)) => {
                        ...
                        if similarity > 0.6 {
                            ...
                            info!(
                                "✅ Found partial match: {} (score: {:.2})",
                                packet.display_name, similarity
                            );
                        }
                    }
...
#[tauri::command]
async fn check_passkey_availability(state: State<'_, AppState>) -> Result<bool, String> {
...
async fn create_passkey(
    ...
    let credential = manager
        .create_passkey(current_user_id, three_word_address)
        .await
    ...
    if let Some(storage) = storage_guard.as_ref() {
        storage
            .add_passkey_credential(&credential, &password)
            .await
...
async fn get_stored_passkey_credentials(
    ...
    let public_credentials: Vec<serde_json::Value> = credentials
        .iter()
        .map(|cred| {
            serde_json::json!({
...
#[tauri::command]
async fn clear_all_identity_data(state: State<'_, AppState>) -> Result<String, String> {
