...
fn bundle_frontend_assets() {
    ...
    let (index_html, styles_css, main_js) = if src_dir.exists() {
        ...
        let index = fs::read_to_string(src_dir.join("index.html")).unwrap_or_else(|e| {
            println!("cargo:warning=Failed to read index.html: {e}");
            get_fallback_index()
        });
        let styles = fs::read_to_string(src_dir.join("styles.css")).unwrap_or_else(|e| {
            println!("cargo:warning=Failed to read styles.css: {e}");
            get_fallback_styles()
        });
        let main = fs::read_to_string(src_dir.join("main.js")).unwrap_or_else(|e| {
    ...
    content.push(format!(
        r#"pub const INDEX_HTML: &str = "{index_escaped}";"#
    ));
    content.push("".to_string());
    content.push(format!(
        r#"pub const STYLES_CSS: &str = "{styles_escaped}";"#
    ));
    content.push("".to_string());
