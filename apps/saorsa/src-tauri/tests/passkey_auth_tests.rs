...
mod tests {
    ...
    async fn test_authenticate_with_passkey() {
        ...
        let result = manager.authenticate_with_passkey(&credential.credential_id).await;
        assert!(result.is_ok());
    ...
    async fn test_authenticate_with_invalid_credential() {
        ...
        let result = manager.authenticate_with_passkey("invalid_credential_id").await;
        assert!(result.is_err());
    }
    ...
    async fn test_multiple_passkeys() {
        ...
        let cred1 = manager.create_passkey("user1", "user1.test.address")
            .await
            .unwrap();
        let cred2 = manager.create_passkey("user2", "user2.test.address")
            .await
            .unwrap();
        ...
        let auth1 = manager.authenticate_with_passkey(&cred1.credential_id).await;
        let auth2 = manager.authenticate_with_passkey(&cred2.credential_id).await;
    ...
    async fn test_stored_credential_serialization() {
        ...
        assert_eq!(credential.three_word_address, deserialized.three_word_address);
    ...
    async fn test_concurrent_authentication() {
        ...
        let task1 = tokio::spawn(async move {
            manager1.authenticate_with_passkey(&cred_id1).await
        });
        
        let manager2 = manager.clone();
        let cred_id2 = credential.credential_id.clone();
        let task2 = tokio::spawn(async move {
            manager2.authenticate_with_passkey(&cred_id2).await
        });
