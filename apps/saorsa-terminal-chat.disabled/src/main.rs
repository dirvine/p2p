...
use anyhow::Result;
use colored::*;
use saorsa_core::{NodeBuilder, P2PNode};
...
async fn main() -> Result<()> {
    ...
    println!(
        "{}",
        "║               🐜 Saorsa P2P Chat (Real Network) 🐜                  ║".cyan()
    );
    println!(
        "{}",
        "║                                                                      ║".cyan()
    ...
    println!(
        "{}",
        "║        Using QUIC, DHT, and Quantum-Resistant Crypto                ║".cyan()
    );
    println!(
        "{}",
        "╚══════════════════════════════════════════════════════════════════════╝".cyan()
...
async fn create_chat_room() -> Result<()> {
    ...
    println!(
        "{}",
        "║  Share this peer ID with friends to let them join!                  ║".cyan()
    );
    println!(
        "{}",
        "║                                                                      ║".cyan()
    ...
    if addresses.iter().any(|a| a.to_string().contains("ip6")) {
        println!(
            "{}",
            "║  Transport: QUIC over IPv6 (native)                                 ║".cyan()
        );
    } else {
        println!(
            "{}",
            "║  Transport: QUIC with automatic tunneling                           ║".cyan()
        );
    }
...
async fn join_chat_room() -> Result<()> {
    ...
    match node.connect_peer(address).await {
        Ok(peer_id) => {
            ...
            node.send_message(&peer_id, "/chat/1.0.0", b"Hello from the chat!".to_vec())
                .await?;
        }
        Err(e) => {
        ...
        match trimmed {
            ...
            _ if !trimmed.is_empty() => {
                ...
                if peers.is_empty() {
                    ...
                    for peer in peers {
                        node.send_message(&peer, "/chat/1.0.0", trimmed.as_bytes().to_vec())
                            .await?;
                    }
