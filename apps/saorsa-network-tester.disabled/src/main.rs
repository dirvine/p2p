...
use anyhow::Result;
use colored::*;
use saorsa_core::{Key, NodeBuilder};
...
async fn main() -> Result<()> {
    ...
    loop {
        ...
        println!(
            "{}",
            "║            🐜 Saorsa P2P Network Tester (Real Stack) 🐜            ║".cyan()
        );
        println!(
            "{}",
            "║                                                                      ║".cyan()
        ...
        println!(
            "{}",
            "║        QUIC Transport, DHT, Tunneling, and ML-KEM/ML-DSA           ║".cyan()
        );
        println!(
            "{}",
            "╚══════════════════════════════════════════════════════════════════════╝".cyan()
...
async fn run_dht_test() -> Result<()> {
    ...
    // Create node
    let node = NodeBuilder::new().with_default_dht().build().await?;
    ...
    let test_data: Vec<(&[u8], &[u8])> = vec![
        ...
        (
            b"large-key",
            b"This is a larger value to test DHT storage capabilities",
        ),
    ];
...
async fn run_address_test() -> Result<()> {
    ...
    let node1 = NodeBuilder::new().with_default_dht().build().await?;
    ...
    let node2 = NodeBuilder::new().with_default_dht().build().await?;
    ...
    if let Some(addr) = addrs1.first() {
        match node2.connect_peer(&addr.to_string()).await {
            ...
            Err(e) => {
                ...
                println!(
                    "  {} This is normal if nodes are on same machine without proper routing",
                    "ℹ️".blue()
                );
            }
        }
...
fn show_test_summary(passed: usize, failed: usize, duration: Duration) {
    ...
    if failed == 0 {
        println!(
            "{}",
            "║           🎉 All tests passed! P2P network is healthy! 🎉          ║".cyan()
        );
    } else if pass_rate >= 80.0 {
        println!(
            "{}",
            "║        ✅ P2P network mostly functional, some issues detected      ║".cyan()
        );
    } else {
        println!(
            "{}",
            "║        ⚠️  P2P network issues detected, check configuration       ║".cyan()
        );
    }
