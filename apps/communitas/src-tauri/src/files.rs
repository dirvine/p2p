...
impl FileManager {
    ...
    pub async fn upload_file(
        ...
        self.uploads.write().await.insert(file_id.clone(), progress.clone());
        ...
        // Update progress
        self.update_upload_progress(&file_id, UploadStatus::Uploading, 0).await?;
        ...
        for (i, chunk_data) in file_data.chunks(self.chunk_size).enumerate() {
            ...
            self.update_upload_progress(&file_id, UploadStatus::Uploading, uploaded as u64).await?;
        }
        ...
        // Store file metadata
        self.files.write().await.insert(file_id.clone(), file_metadata.clone());
        
        // Update user quota
        *used += metadata.len();
        
        // Mark upload complete
        self.update_upload_progress(&file_id, UploadStatus::Complete, metadata.len()).await?;
    ...
    pub async fn download_file(&self, file_hash: String, destination: String) -> Result<()> {
        ...
        let file_metadata = files.values()
        ...
        for chunk_hash in &file_metadata.chunks {
            let chunk = chunks.get(chunk_hash)
    ...
    pub async fn list_user_files(&self, user: String) -> Result<Vec<FileMetadata>> {
        let files = self.files.read().await;
        let user_files: Vec<FileMetadata> = files.values()
    ...
    pub async fn list_group_files(&self, group_id: String) -> Result<Vec<FileMetadata>> {
        let files = self.files.read().await;
        let group_files: Vec<FileMetadata> = files.values()
